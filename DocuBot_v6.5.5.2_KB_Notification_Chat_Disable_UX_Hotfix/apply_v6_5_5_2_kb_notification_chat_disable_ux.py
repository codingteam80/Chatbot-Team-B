...
VERSION = "v6.5.5.2"
SIMULATE = os.getenv("DOCUBOT_V6552_SIMULATE", "").strip() == "1"
FORCE_FAIL = os.getenv("DOCUBOT_V6552_FORCE_VALIDATION_FAIL", "").strip() == "1"
RESULT_REL = "logs/v6_5_5_2_notification_chat_ux"
MARKER_REL = "KB_NOTIFICATION_CHAT_DISABLE_UX_HOTFIX_APPLIED_v6.5.5.2.txt"
OLD_MANIFEST_REL = "FINAL_COMPONENT_MANIFEST_v6.5.5.1.json"
NEW_MANIFEST_REL = "FINAL_COMPONENT_MANIFEST_v6.5.5.2.json"
...
def verify_baseline(root: Path):
    ...
    for rel, wanted in (BASELINE.get("required_existing_sha256") or {}).items():
        p = root / rel
        if not p.is_file():
            problems.append(f"missing v6.5.5.1 baseline file: {rel}")
        else:
    ...
    for rel in BASELINE.get("required_absent") or []:
        if (root / rel).exists():
            problems.append(f"expected absent on clean v6.5.5.1 baseline: {rel}")
    return not problems, problems
...
def fail_restore(root, history, state, message, code):
    print("[FAIL]", message)
    print("[ROLLBACK] Restoring exact v6.5.5.1 files...")
...
def main():
    ...
    print("=" * 104)
    print("DocuBot v6.5.5.2 - KB Notification Polish + Chat Disable UX Hotfix")
    print("=" * 104)
    print("[INFO] Scope: presentable success toast + disable chat editor/send during KB update.")
    print("[INFO] Existing v6.5.5.1 incremental KB/retrieval architecture is not changed.")
    ...
    if not ok:
        print("[FAIL] Project is not the exact supported v6.5.5.1 baseline. No files changed.")
        for x in problems:
            print(" -", x)
        return 3
    print(f"[PASS] Exact v6.5.5.1 baseline verified ({len(BASELINE['required_existing_sha256'])} covered files).")
    ...
    history = root / "evidence" / "release_history" / f"pre_v6.5.5.2_{stamp}"
    ...
    try:
        ...
        marker.write_text(
            "\n".join([
                ...
                "Base: exact verified v6.5.5.1",
                "Success toast polished: YES",
                "Success toast counts: Added / Updated / Deleted only",
                "Success icon: YES",
                "Chat textarea disabled during KB update: YES",
                "Chat send disabled during KB update: YES",
                "Server-side chat submit guard during KB update: YES",
    ...
    validation = run_cmd(root, [py, "-X", "utf8", "-u", "scripts/validate_v6_5_5_2_kb_notification_chat_disable_ux.py"])
    print(validation.stdout or "", end="")
    if FORCE_FAIL:
        validation = subprocess.CompletedProcess(validation.args, 99, stdout=(validation.stdout or "") + "\nFORCED TEST FAILURE\n")
    if validation.returncode != 0:
        return fail_restore(root, history, state, "v6.5.5.2 notification/chat UX validation failed.", 7)
    ...
    summary = {
        ...
        "success_notification_polished": True,
        "success_notification_icon": "success",
        "technical_details_preserved_in_logs": True,
        "chat_textarea_disabled_during_kb_update": True,
        "chat_send_disabled_during_kb_update": True,
        "chat_server_submit_guard_during_kb_update": True,
        "chat_layout_position_changed": False,
        ...
        "simulation": SIMULATE,
        "next": "Restart DocuBot and visually verify one KB update: chat must be disabled while running and the success toast must be clean/readable.",
    }
    summary_path = outdir / f"v6.5.5.2_notification_chat_ux_{stamp}.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    result_zip = outdir / f"DocuBot_v6.5.5.2_KB_Notification_Chat_Disable_UX_Hotfix_Result_{stamp}.zip"
    evidence = [
        summary_path,
        root / MARKER_REL,
        root / NEW_MANIFEST_REL,
        root / "logs" / "v6_5_5_2_notification_chat_ux" / "validation_latest.json",
        root / "docs" / "KB_NOTIFICATION_CHAT_DISABLE_SMOKE_v6.5.5.2.txt",
    ]
    ...
    print("[PASS] v6.5.5.2 installation completed.")
