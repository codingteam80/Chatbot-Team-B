...
def _kb_update_success_message(update_result):
    """Return a compact two-line success toast; technical detail stays in logs."""
    ...
    # Markdown's two trailing spaces create a real line break inside one
    # paragraph. This avoids Streamlit laying separate paragraph blocks out
    # awkwardly in the narrow toast container.
    return (
...
if flash_message:
    ...
    # the welcome/composer region away from its normal position. Success gets
    # a clear visual icon; failures keep a warning icon.
    toast_icon = "✅" if flash_message.startswith("**Knowledge Base") else "⚠️"
