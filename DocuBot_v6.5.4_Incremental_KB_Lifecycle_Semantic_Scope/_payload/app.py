...
try:
    ...
    st.session_state.kb_update_reason = (
        f"Knowledge-base status check failed: {type(kb_status_error).__name__}: {kb_status_error}"
    )
    st.session_state.kb_status_error = st.session_state.kb_update_reason
...
if st.session_state.kb_outdated:
    ...
    if not web_kb_update_allowed:
        ...
        if st.session_state.get("kb_update_mode") == "status_error":
        ...
        if "kb_update_in_progress" not in st.session_state:
            st.session_state.kb_update_in_progress = False

        update_in_progress = bool(st.session_state.get("kb_update_in_progress", False))
        if update_in_progress:
            st.info("Knowledge-base update worker is already running for this browser session.")

        update_clicked = st.button(
            "Update Knowledge Base",
            key="kb_smart_update",
            disabled=(not rebuild_confirmed or update_in_progress),
        )
        if update_clicked:
            # The Streamlit process never owns the cross-process KB lock.
            # It only launches the isolated worker. The worker is the sole lock
            # owner from preflight through rebuild, semantic verification and
            # KB Health, preventing the v6.5.3 parent/child self-lock failure.
            st.session_state.kb_update_in_progress = True
            try:
                update_mode_label = st.session_state.get("kb_update_mode")
                spinner_text = (
                    "Applying only new/modified/deleted knowledge-base changes and verifying KB Health..."
                    if update_mode_label == "incremental"
                    else "Updating technical knowledge base safely and verifying KB Health..."
                )
                with st.spinner(spinner_text):
                    update_result = launch_kb_update_subprocess(
            ...
            if update_result.get("overall") == "PASS":
                build_action = str(update_result.get("build_action") or "")
                if build_action == "transactional_incremental":
                    details = update_result.get("build_details") or {}
                    st.success(
                        "Knowledge base updated incrementally and verified. "
                        f"Unchanged skipped: {details.get('unchanged_skipped', 0)}; "
                        f"changed chunks embedded: {details.get('embedded_changed_chunks', 0)}. "
                        "Qdrant/BM25, the derived MISRA semantic Rule index, and KB Health all passed."
                    )
                else:
                    st.success(
                        "Knowledge base updated and verified. Qdrant/BM25, the derived "
                        "MISRA semantic Rule index, and KB Health all passed."
                    )
                st.rerun()
            else:
                ...
                st.error(
                    "Knowledge base update did not complete. The guarded workflow "
                    f"stopped at stage '{stage}'. {message}"
                )
                details = []
                if update_result.get("report_path"):
                    details.append(f"Result report: {update_result['report_path']}")
                if update_result.get("console_log"):
                    details.append(f"Console log: {update_result['console_log']}")
                handoff = update_result.get("launcher_lock_handoff") or {}
                if handoff.get("action") and handoff.get("action") != "none":
                    details.append(f"Lock handoff: {handoff.get('action')}")
                if details:
                    st.info("\n\n".join(details))
