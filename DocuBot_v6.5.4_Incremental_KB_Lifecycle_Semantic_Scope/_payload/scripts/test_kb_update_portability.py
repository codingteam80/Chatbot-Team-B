...
def main() -> int:
    ...
    try:
        ...
        check("Read-only update plan executes", isinstance(plan, dict) and plan.get("mode") in {"noop", "incremental", "full_rebuild"}, {"mode": plan.get("mode"), "reason": plan.get("reason")})
    ...
    check("UI advertises unchanged-file skip behavior", "Unchanged files will not be re-embedded" in app_source)
    ...
    incremental_source = (ROOT / "scripts" / "update_qdrant_incremental.py").read_text(encoding="utf-8")
    check("Planner exposes incremental mode", 'mode = "incremental"' in smart_source)
    check("Incremental updater embeds changed documents only", "added", "updated" in incremental_source and "unchanged_skipped" in incremental_source)
    check("Incremental updater has source stability guard", "source_stability_guard" in incremental_source and "_source_state_matches" in incremental_source)
