...
def get_kb_update_preflight(documents=None, *, requires_embedding: bool = True):
    ...
    Ollama availability does not unnecessarily block a mutation that creates no
    new vectors.
    """
    ...
    elif not requires_embedding:
        notes.append("Ollama embedding preflight skipped because this update creates no new vectors.")
...
def get_update_plan():
    ...
    if _manifest_requires_full_rebuild(old_manifest):
        ...
        full_rebuild_reason = "The active Qdrant index is missing or empty while documents are tracked."
    elif not old_manifest and documents:
        full_rebuild_reason = "No active tracking manifest exists yet; establish one certified baseline index first."
...
def _plan_requires_embedding(plan) -> bool:
    if plan.get("mode") == "full_rebuild":
        return True
    changes = plan.get("changes") or {}
    return bool(changes.get("added") or changes.get("updated"))


_LAST_BUILD_RESULT = {}


def get_last_build_result():
    return dict(_LAST_BUILD_RESULT)


def smart_build():
    """Safely apply the canonical production KB plan without crashing the UI.

    Normal source add/modify/delete operations are transactional *incremental*
    updates. Unchanged documents remain in Qdrant and are not re-embedded.
    Full rebuilds are reserved for incompatible/missing/corrupt index identity.
    """
    ...
    try:
        ...
        if plan["mode"] == "noop":
            ...
            _LAST_BUILD_RESULT = {"ok": True, "mode": "noop", "action": "noop"}
            return True
        ...
        if plan["mode"] == "incremental":
            print("Action           : transactional incremental Qdrant update")
            print("Policy           : unchanged=SKIP; new/modified/deleted=PROCESS")
            try:
                from scripts.update_qdrant_incremental import build_qdrant_incremental
            except Exception as error:
                log_path = _write_failure_log("incremental build-module import", error=error)
                print("[FAIL] Incremental KB update could not start. Previous index was preserved.")
                print(f"Diagnostic log: {log_path}")
                _LAST_BUILD_RESULT = {
                    "ok": False,
                    "mode": "incremental",
                    "action": "transactional_incremental",
                    "error": f"{type(error).__name__}: {error}",
                }
                return False

            detail = build_qdrant_incremental(plan)
            _LAST_BUILD_RESULT = dict(detail or {})
            _LAST_BUILD_RESULT.setdefault("mode", "incremental")
            _LAST_BUILD_RESULT.setdefault("action", "transactional_incremental")
            if not detail.get("ok"):
                log_path = _write_failure_log(
                    "transactional incremental update",
                    details=detail.get("error") or "build_qdrant_incremental returned failure",
                )
                print(f"Diagnostic log: {log_path}")
                return False
        else:
        ...
        if post_plan["mode"] != "noop":
            ...
            print("[FAIL] Update finished but the committed KB did not pass the post-check.")
            print(f"Diagnostic log: {log_path}")
            _LAST_BUILD_RESULT["ok"] = False
            _LAST_BUILD_RESULT["post_plan_mode"] = post_plan.get("mode")
            _LAST_BUILD_RESULT["post_plan_reason"] = post_plan.get("reason")
            return False

        print("[PASS] Production Qdrant v4 + BM25 update completed successfully.")
        _LAST_BUILD_RESULT["ok"] = True
        return True
