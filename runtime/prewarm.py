...
def prewarm_retrieval_sync() -> dict[str, Any]:
    ...
    try:
        ...
        # Warm both document and query embedding paths. Qwen3 applies a query
        # instruction on the query path, so warming only document embeddings can
        # leave the first real Rule lookup paying a separate initialization cost.
        embed_model.get_text_embedding("docubot runtime readiness")
        query_embed = getattr(embed_model, "get_query_embedding", None)
        if callable(query_embed):
            query_embed("MISRA requirement lookup readiness")
        ...
        # Load the small semantic Rule metadata/vector files as part of startup
        # prewarm as well. This does not execute MultiQuery and never changes
        # production Qdrant/BM25; it removes lazy file/model setup from the
        # first user question on a newly started PC.
        try:
            from retrieval.semantic_rule_resolver import SemanticRuleResolver
            semantic_resolver = SemanticRuleResolver(
                embedding_model=embed_model,
                reranker=None,
            )
            semantic_resolver._ensure_loaded()
        except Exception:
            # The existing build/load step above remains authoritative. Resolver
            # metadata warmup is an optimization and must not make startup fail.
            pass
