...
class AnswerService:
    ...
    def _deterministic_structured_major_family_answer(results):
        """Clarify a bare major Rule identifier from the indexed family."""
        ...
        for item in results or []:
            ...
            if not major or not members:
                continue
            groups = AnswerService._compress_rule_ids_for_display(members)
            family_label = ", ".join(groups) if groups else ", ".join(f"Rule {m}" for m in members)
            return (
                f"There is no single **Rule {major}** entry. "
                f"The available source contains the Rule {major} family: **{family_label}**. "
                f"Please specify the exact Rule {major}.x you want explained or summarized."
            )
        return ""
