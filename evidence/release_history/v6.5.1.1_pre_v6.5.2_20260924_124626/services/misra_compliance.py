...
class MisraComplianceMode:
    ...
    def yes_no_intent(cls, question: str) -> str:
        ...
        permission_clean = re.sub(
            r"^(?:in|under)\s+misra(?:\s+c)?\s*,?\s*",
            "",
            clean,
        )
        if (
            re.match(r"^(?:does|do|did|is|are|was|were|can|could|should|may)\b", permission_clean)
            and re.search(r"\b(?:allow|allows|allowed|permit|permits|permitted|acceptable|valid|okay|ok|use|used|using|have|leave|keep|place|placed|placing|omit|omits|omitted|omitting|exclude|excluded|skip|skipped|remove|removed|jump|jumps|jumping|target|targets|targeting)\b", permission_clean)
        ) or re.match(r"^(?:pwede|puwede)\s+ba\b", permission_clean) or re.search(
