...
def main() -> int:
    ...
    # Full active-project-source syntax check.
    # Environment/package trees are intentionally excluded: they are third-party
    # installations, not DocuBot source, and may contain compatibility source
    # that is not parseable by the project's current Python interpreter.
    syntax_errors = []
    py_count = 0
    excluded_top_levels = {
        "logs", "evidence", "venv", ".venv", "env", ".git",
        ".pytest_cache", ".mypy_cache", ".ruff_cache", ".tox",
        "node_modules",
    }
    for path in ROOT.rglob("*.py"):
        rel = path.relative_to(ROOT)
        if rel.parts and rel.parts[0].casefold() in excluded_top_levels:
            continue
    ...
    report = {
        "version": "v6.4.85",
        "phase": "manual_qa_correctness_and_formatting_hotfix_validation_v6.4.85.1",
