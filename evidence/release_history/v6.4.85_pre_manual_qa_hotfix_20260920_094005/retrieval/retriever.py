...
class CompanyRetriever:
    ...
    def _get_chroma(self):
        ...
        # Compatibility method/attribute name retained so the certified
        # retrieval pipeline does not need a broad API rewrite.  The finalized
        # production backend is Qdrant only.
    ...
    def _augment_compound_candidates(
        ...
        for facet_index, facet_query in enumerate(facet_queries, start=1):
            ...
            vector_results = self._get_chroma().search(
                facet_query,
                per_facet_top_k
            )
