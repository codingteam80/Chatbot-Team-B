...
class MisraComplianceMode:
    ...
    def is_request(cls, question: str, current_topic: str = "") -> bool:
        ...
        # Preserve the existing exact Rule/Directive explanation path unless
        # the user is actually asking for a compliance assessment.
    ...
    def yes_no_intent(cls, question: str) -> str:
        ...
        if (
            ...
            and re.search(r"\b(?:allow|allows|allowed|permit|permits|permitted|acceptable|valid|okay|ok|use|used|using)\b", clean)
    ...
    def deterministic_yes_no_answer(
        ...
        support: list[str] = []
        for item, statement in zip(candidates, statements):
    ...
    def exact_reference_evidence(
        ...
        expansion helper.
        """
        refs = list(_REF_RE.finditer(str(question or "")))
        if not refs or not records:
            return []
        wanted = []
        for match in refs:
            kind = str(match.group("kind") or "rule").casefold()
            identifier = str(match.group("identifier") or "").strip()
            key = ("directive" if kind.startswith("dir") else "rule", identifier)
            if identifier and key not in wanted:
                wanted.append(key)
