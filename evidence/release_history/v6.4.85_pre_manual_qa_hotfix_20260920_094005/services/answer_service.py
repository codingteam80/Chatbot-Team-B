...
class AnswerService:
    ...
    def _deterministic_structured_topic_list_answer(results):
        ...
        heading = f"### {display_family.capitalize()} rules\n\n" if display_family else ""
        body = "\n".join(
    ...
    def _detect_answer_focus(
        ...
        if (
            len([ref for ref in structured_references if ref.kind in {"rule", "directive"}]) >= 2
            and re.search(
                r"\b(?:compare|comparison|contrast|difference|different|versus|vs\.?|and)\b",
                clean,
                flags=re.IGNORECASE,
            )
        ):
        ...
        if structured_reference:
            ...
            if structured_reference.kind in {"rule", "directive"}:
                ...
                if re.search(r"\bcategory\b", structured_intent_text):
                    return (
...
"""
    ...
    def _structured_detail_label(question: str, resolved_question: str = ""):
        ...
        if re.search(r"\bcategory\b", text):
            return "Category"
        if re.search(
            r"\b(?:apply\s+to|applies\s+to|applicable\s+to|applicability|c\s+versions?|c\s+standards?)\b",
            text,
...
"""
    ...
    def ask(
        ...
        if precomputed_structured_comparison_answer:
        ...
        try:
            ...
            if route_decision.route == "deterministic":
                ...
                if answer:
                    ...
                    if precomputed_structured_list_answer:
