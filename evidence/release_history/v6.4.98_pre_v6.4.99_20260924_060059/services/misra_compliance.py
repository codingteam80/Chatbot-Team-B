...
class MisraComplianceMode:
    ...
    def _natural_requirement_cues(cls, question: str) -> list[str]:
        ...
        # Unused function parameters.  Support English and common Tagalog/Taglish
        # wording so the source-language Rule body can be selected before a
        # mixed-language reranker has a chance to suppress it.
        if (
            re.search(r"\bunused\b.{0,28}\bparameters?\b|\bparameters?\b.{0,28}\bunused\b", clean)
            or re.search(r"\b(?:hindi|di)\s+(?:ginagamit|nagagamit|used)\b.{0,30}\bparameters?\b", clean)
            or re.search(r"\bparameters?\b.{0,30}\b(?:hindi|di)\s+(?:ginagamit|nagagamit|used)\b", clean)
        ):
