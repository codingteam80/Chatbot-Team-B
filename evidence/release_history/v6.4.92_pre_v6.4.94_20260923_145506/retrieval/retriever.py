...
class CompanyRetriever:
    ...
    def _retrieve_structured_toolchain_configuration_section(
        ...
        """Resolve compiler/toolchain switch/option wording to source configuration guidance.

        Natural engineering language often uses *compiler switch*, *compiler
        option*, *flag*, or *setting* for command-line/tool configuration.
        This must remain distinct from both the C ``switch`` statement family
        and preprocessor ``#if``/``#elif`` rules.

        The target Section is discovered from indexed Section metadata by its
        own title (compiler + configuration); no Rule identifier is hardcoded.
        """
        ...
        has_configuration_term = bool(
            re.search(
                r"\b(?:switch|switches|option|options|flag|flags|"
                r"setting|settings|configuration|configure|configured)\b",
                semantic_query,
            )
        )
