...
class OllamaClient:
    ...
    def _wait_for_fast_recovery(self):
    ...
    def generate_structured_json(self, prompt: str, schema: dict):
        """Generate one JSON object under an Ollama JSON schema.

        This low-level call is intentionally limited to retrieval-rewrite
        generation. It keeps temperature at zero and requests non-streaming,
        non-thinking output while preserving the same model and keep-alive
        policy as normal generation. Callers retain a guarded text fallback.
        """

        self._wait_for_fast_recovery()
        url = f"{str(EMBED_OLLAMA_URL).rstrip('/')}/api/chat"
        payload = {
            "model": self.model_name,
            "messages": [{"role": "user", "content": str(prompt or "")}],
            "stream": False,
            "think": False,
            "format": dict(schema or {}),
        ...
        def _post(body):
            ...
            if not isinstance(content, str) or not content.strip():
                raise RuntimeError("Ollama structured response did not contain message.content")
            return content.strip()

        try:
            return _post(payload)
        except Exception as error:
            # Older Ollama builds may not accept the optional `think` field.
            # Retry once without it only for a request-shape/HTTP-400 failure;
            # the response schema and temperature=0 remain enforced.
            status = getattr(getattr(error, "response", None), "status_code", None)
            if status == 400 and "think" in payload:
                compatibility_payload = dict(payload)
                compatibility_payload.pop("think", None)
                return _post(compatibility_payload)
            if not _is_transient_connection_error(error):
                raise
            time.sleep(0.4)
            return _post(payload)
