from config.settings import (
    ...
    INTERNAL_REASONING_GUIDANCE_ENABLED,
)
...
_SILENT_GROUNDED_REASONING_GUIDANCE = """
==================================================
SILENT EVIDENCE CHECK
==================================================
Before writing the final answer, silently perform this evidence check:
1. Identify the exact requested fact type, scope, and polarity.
2. Locate the minimum COMPANY KNOWLEDGE statements that directly support it.
3. Distinguish requirement/statement text from rationale, example, appendix,
   neighboring rules, and conversation history.
4. Verify that each planned factual claim is supported at the same scope.
5. Remove unsupported claims, invented identifiers, and unnecessary expansion.

Do not reveal this checklist, chain-of-thought, scratch work, hidden reasoning,
or intermediate analysis. Return only the final grounded answer.
"""


_ANSWER_BEHAVIOR_FEW_SHOTS = """
==================================================
BEHAVIOR-ONLY FEW-SHOT EXAMPLES
==================================================
These examples teach answer behavior only. They are NOT COMPANY KNOWLEDGE and
must never be treated as factual evidence or copied as domain facts.

Example A — permission polarity
Evidence pattern: "Operation A shall not be used."
Question pattern: "Is Operation A allowed?"
Desired answer pattern: "No. Operation A shall not be used."

Example B — classification
Evidence pattern: "Item B — Category: Advisory"
Question pattern: "What category is Item B?"
Desired answer pattern: "Advisory."

Example C — rationale focus
Evidence pattern: "Rationale: Condition C prevents unintended state changes."
Question pattern: "Why does Item C exist?"
Desired answer pattern: "Condition C prevents unintended state changes."

Example D — unsupported request
Evidence pattern: COMPANY KNOWLEDGE does not state the requested fact.
Desired answer pattern: "Information not found in company knowledge base."
"""


_MISRA_BEHAVIOR_FEW_SHOTS = """
==================================================
MISRA BEHAVIOR-ONLY FEW-SHOT EXAMPLES
==================================================
These are abstract behavior examples, not MISRA facts and not citable evidence.

Example A — direct permission question
Citable requirement pattern: "The feature shall not be used."
Question pattern: "Can I use the feature?"
Desired answer shape: start with "No." and give the brief source-backed reason.

Example B — exact rationale question
Evidence contains a Rule statement plus a labeled Rationale.
Question pattern: "Why does this Rule exist?"
Desired answer shape: return the Rationale, not the neighboring Rule statement.

Example C — visible-code uncertainty
A Rule is relevant, but the shown code does not establish a required fact.
Desired answer shape: "Needs more context" rather than inventing a violation.

Example D — comparison
Two exact Rule statements are requested.
Desired answer shape: compare each Rule from its own statement and do not
substitute rationale/examples for the Rule text.
"""


_MULTI_QUERY_BEHAVIOR_FEW_SHOTS = """
==================================================
BEHAVIOR-ONLY MULTIQUERY EXAMPLES
==================================================
These examples teach retrieval-rewrite behavior only. They are not answers and
must not add facts to the user's question.

Example A
Input: What restrictions apply to goto labels?
Good structured output: {{"queries":["requirements for labels referenced by goto statements","constraints on goto target label scope"]}}

Example B
Input: Why should persistent side effects be avoided in the right operand of && or ||?
Good structured output: {{"queries":["rationale for persistent side effects in the right operand of && or ||","purpose of avoiding persistent side effects in logical &&/|| right operands"]}}

Example C
Input: Pwede ba recursion sa MISRA?
Good structured output: {{"queries":["whether recursion is permitted by MISRA","MISRA permission requirements for recursion"]}}

Bad behavior to avoid:
- inventing an unmentioned Rule/Directive/Section number;
- broadening to generic coding best practices;
- changing permission, rationale, comparison, or list intent into another intent;
- dropping technical operators, negation, scope words, or exact identifiers.
"""
...
__DOCUBOT_REASONING_GUIDANCE__
==================================================
ANSWER FORMAT
==================================================
1. Answer directly.
...
2. Preserve exact Rule/Directive/Section identifiers, commands, paths, product
   names, acronyms, numbers, standards, dates, and technical operators such as
   &&/||.
...
7. Return only a JSON object with one key named `queries`. Its value must contain
   exactly {variant_count} strings. Do not repeat the original query verbatim and
   do not add commentary or any other JSON keys.

__DOCUBOT_MULTIQUERY_FEW_SHOTS__
...
# =====================================================
# OPTIONAL PROMPT STRATEGY ACTIVATION
# =====================================================
# Behavior-only examples are deliberately outside COMPANY KNOWLEDGE. The
# explicit warning in each block prevents them from becoming factual evidence.
# Both features are A/B-toggleable from environment variables in settings.py.
if INTERNAL_REASONING_GUIDANCE_ENABLED:
    SYSTEM_PROMPT = SYSTEM_PROMPT.replace(
        "__DOCUBOT_REASONING_GUIDANCE__",
        _SILENT_GROUNDED_REASONING_GUIDANCE.strip(),
    )
else:
    SYSTEM_PROMPT = SYSTEM_PROMPT.replace("__DOCUBOT_REASONING_GUIDANCE__", "")

if FEW_SHOT_PROMPTING_ENABLED:
    ANSWER_TEMPLATE = ANSWER_TEMPLATE.replace(
        "__DOCUBOT_ANSWER_FEW_SHOTS__",
        _ANSWER_BEHAVIOR_FEW_SHOTS.strip(),
    )
    MISRA_COMPLIANCE_TEMPLATE = MISRA_COMPLIANCE_TEMPLATE.replace(
        "__DOCUBOT_MISRA_FEW_SHOTS__",
        _MISRA_BEHAVIOR_FEW_SHOTS.strip(),
    )
    MULTI_QUERY_RETRIEVAL_PROMPT = MULTI_QUERY_RETRIEVAL_PROMPT.replace(
        "__DOCUBOT_MULTIQUERY_FEW_SHOTS__",
        _MULTI_QUERY_BEHAVIOR_FEW_SHOTS.strip(),
    )
else:
    ANSWER_TEMPLATE = ANSWER_TEMPLATE.replace("__DOCUBOT_ANSWER_FEW_SHOTS__", "")
    MISRA_COMPLIANCE_TEMPLATE = MISRA_COMPLIANCE_TEMPLATE.replace(
        "__DOCUBOT_MISRA_FEW_SHOTS__",
        "",
    )
