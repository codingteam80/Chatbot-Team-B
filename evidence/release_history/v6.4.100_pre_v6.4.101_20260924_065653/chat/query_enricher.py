...
class QueryEnricher:
    ...
    def __init__(self):
        ...
        self.eligibility_patterns = [
            ...
            r"\b(?:eligible|eligibility|qualified|qualification|qualify|qualifies|covered)\b",
    ...
    def _extract_eligibility_subject(
        ...
        patterns = [
            # Front-loaded approval relation wording. Mixed-language source
            # attribution such as "approval role ayon sa <document/topic>"
            # is semantically equivalent to "approval role according to
            # <document/topic>". Keep this domain-neutral and extract only
            # the actual subject/topic after the relation phrase.
            r"^(?:the\s+)?(?:approval|approver|approving)\s+(?:role|authority|entity|position|person)\s+(?:(?:according\s+to|under|in|for|ayon\s+sa)\s+)?(.+)$",
        ...
        for candidate in candidates:
            ...
            for pattern in patterns:
                ...
                # A canonical pattern must never anchor relation enrichment to
                # a determiner or bare relation label. If such a generic token
                # is captured, continue to the next pattern/candidate instead
                # of generating noisy terms like "the approval authority".
                if subject.casefold() in {
                    "the", "a", "an", "approval", "approver",
                    "role", "authority", "entity", "position",
                    "person",
                }:
                    continue
    ...
    def _is_technical_permission_question(
        self,
        lowered_question: str
    ) -> bool:
        """Return True for technical allowability questions, not role authorization.

        Words such as ``allowed`` and ``permitted`` occur in both access-control
        policy questions and technical standards.  A question about whether a
        language construct, function, parameter, pointer, operator, declaration,
        header, or coding practice is allowed must stay anchored to that technical
        construct instead of expanding into roles/access/authorization vocabulary.
        """

        clean = re.sub(r"\s+", " ", str(lowered_question or "").strip().lower())
        if not clean:
            return False

        if not re.search(
            r"\b(?:allowed|allowable|permitted|permission|acceptable|okay|ok|"
            r"pwede|puwede|bawal|prohibited|forbidden)\b",
            clean,
        ):
            return False

        # Explicit actor/role authorization remains on the policy path.
        if re.search(
            r"\b(?:who|which\s+(?:person|people|role|roles|team|teams|user|users|"
            r"employee|employees|group|groups))\b",
            clean,
        ) or re.search(
            r"\b(?:access|account|credential|role|roles|authorization|authorize|"
            r"approver|approval\s+authority)\b",
            clean,
        ):
            return False

        technical_signal = re.search(
            r"\b(?:misra|coding|code|function|functions|parameter|parameters|argument|"
            r"arguments|declaration|declarations|switch|goto|label|labels|recursion|"
            r"recursive|pointer|pointers|string|operator|operators|macro|macros|"
            r"header|library|stdarg|compiler|expression|expressions|array|arrays|"
            r"return|loop|loops|struct|union|enum|typedef)\b",
            clean,
        )
        return bool(technical_signal or "&&" in clean or "||" in clean)
    ...
    def _is_eligibility_question(
        ...
        if self._is_technical_permission_question(lowered_question):
            return False
