...
class MisraComplianceMode:
    ...
    def _unused_function_parameter_concept(cls, question: str) -> bool:
        ...
        direct_unused_terms = {"unused", "unreferenced", "ignored"}
        negation_terms = {"never", "not", "without", "hindi", "di", "walang"}

        def usage_term(token: str) -> bool:
            return (
                token in {"use", "used", "using", "gamit", "gamitin"}
        ...
        for pos in entity_positions:
            ...
            for idx, token in enumerate(window):
                ...
                # Keep precision when "unused" clearly modifies a different
                # object (for example an unused *return value*) that merely
                # happens to occur near the word parameter.
                if {"return", "value", "label", "macro", "tag", "typedef", "variable"}.intersection(between):
                    continue
                return True
            ...
            for neg_pos in neg_positions:
                for use_pos in usage_positions:
                    if abs(use_pos - neg_pos) <= 4 and max(abs(neg_pos - pos), abs(use_pos - pos)) <= 10:
                        return True
