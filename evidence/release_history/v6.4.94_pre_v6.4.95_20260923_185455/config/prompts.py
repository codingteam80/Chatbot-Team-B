...
Create {variant_count} concise alternative retrieval queries for the user's
question. These are search formulations only; do not answer the question.

Rules:
1. Preserve exact Rule/Directive/Section identifiers, codes, commands, paths,
   product names, acronyms, numbers, and standards.
2. Keep the original intent and requested fact type unchanged.
3. Use materially different wording only when it can improve retrieval recall.
4. Do not add facts, assumptions, examples, synonyms with a different technical
   meaning, or outside knowledge.
5. For ambiguous terminology, do not guess the intended meaning.
6. Return exactly one query per line with no numbering, labels, or commentary.
