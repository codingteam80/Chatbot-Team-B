...
# =====================================================
# MULTI-QUERY RETRIEVAL — CONTROLLED EXPERIMENT
# =====================================================
# Kept OFF in the production baseline until the Golden/RAGAS benchmark shows
# a measurable recall gain without unacceptable precision or latency loss.
# The prompt/settings are centralized now so the A/B experiment does not need
# ad-hoc code edits later.
MULTI_QUERY_RETRIEVAL_ENABLED = _env_bool("DOCUBOT_MULTI_QUERY", False)
try:
    MULTI_QUERY_VARIANT_COUNT = max(2, min(5, int(os.getenv("DOCUBOT_MULTI_QUERY_VARIANTS", "3"))))
except (TypeError, ValueError):
