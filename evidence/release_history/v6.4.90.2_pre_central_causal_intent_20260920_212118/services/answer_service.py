...
class AnswerService:
    ...
    def _augment_reason_cross_reference_context(
        ...
        if not re.search(
            r"\b(?:why|rationale|reason|purpose|justification|bakit|dahilan)\b",
            clean_question,
        ):
    ...
    def _detect_answer_focus(
        ...
        if structured_reference:
            ...
            if (
                structured_reference.kind in {"rule", "directive"}
                and re.search(
                    r"\b(?:why|rationale|reason|purpose|justification|"
                    r"basis|motivation|bakit|dahilan|layunin)\b",
                    f"{clean} {resolved_clean.lower()}",
                    flags=re.IGNORECASE,
                )
                and not re.search(
...
"""
    ...
    def ask(
        ...
        if precomputed_named_section_answer:
