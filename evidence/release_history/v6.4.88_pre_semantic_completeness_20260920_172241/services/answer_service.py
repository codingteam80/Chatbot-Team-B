...
class AnswerService:
    ...
    def _first_meaningful_rationale_sentence(raw_text: str):
        """Return the first real rationale sentence, skipping list markers such as ``1.``."""

        text = re.sub(r"\s+", " ", str(raw_text or "")).strip()
        if not text:
            return ""

        # MISRA rationale sections are sometimes numbered:
        #   Rationale
        #   1.
        #   The use of ...
        # Treat the number as an item marker, never as the answer.
        text = re.sub(
            r"^\s*(?:\(?\d+\)?(?:[.)]|\s+))\s*",
            "",
            text,
