...
class SemanticRuleResolver:
    ...
    def _embed_queries(self, queries: list[str]) -> np.ndarray:
        ...
        if callable(batch):
            ...
            rows = [model.get_query_embedding(query) for query in queries]
        matrix = np.asarray(rows, dtype=np.float32)
        if matrix.ndim != 2 or matrix.shape[0] != len(queries) or matrix.shape[1] != self._vectors.shape[1]:
        ...
        *,
        ...
        try:
            ...
            if self.reranker is not None:
                ranked = self.reranker.rerank(original, [dict(item) for item in to_rerank])
            else:
                ranked = sorted(to_rerank, key=lambda item: item.get("semantic_score", 0.0), reverse=True)
                for item in ranked:
            ...
            second_rerank = float((second or {}).get("rerank_score", 0.0) or 0.0)
            rerank_margin = top_rerank - second_rerank
            semantic_rank = int(top.get("semantic_rank", 999) or 999)
            semantic_score = float(top.get("semantic_score", 0.0) or 0.0)
            ...
            # Precision-first acceptance.  The existing 0.55 reranker threshold
            # remains the floor.  Clear semantic/reranker agreement can finalize
            # early; otherwise the result is explicitly ambiguous and the caller
            # may widen recall with MultiQuery or the established hybrid path.
            accepted = False
            if top_rerank >= float(MIN_RETRIEVAL_SCORE) and semantic_rank <= 3:
                if top_rerank >= 0.82 and rerank_margin >= 0.025:
                    accepted = True
                elif top_rerank >= 0.68 and rerank_margin >= 0.060:
                    accepted = True
                elif top_rerank >= float(MIN_RETRIEVAL_SCORE) and rerank_margin >= 0.100 and semantic_rank == 1:
                    accepted = True
                elif semantic_rank == 1 and semantic_margin >= 0.035 and top_rerank >= 0.62 and rerank_margin >= 0.040:
                    accepted = True
