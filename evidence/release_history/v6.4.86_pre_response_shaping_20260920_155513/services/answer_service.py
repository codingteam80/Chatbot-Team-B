...
class AnswerService:
    ...
    def _deterministic_structured_comparison_answer(results):
        ...
        relation = "\n\n**Difference:** " + " ".join(
            f"{reference} addresses this requirement: {statement}."
            for reference, statement in anchors
        )
        return heading + bullets + relation

    @staticmethod
    def _deterministic_structured_multi_reference_answer(results):
        """Explain multiple explicit Rule/Directive anchors without comparing them."""
        ...
        for item in results or []:
            ...
            if rationale_match:
                ...
                sentence = re.split(r"(?<=[.!?])\s+", rationale_text, maxsplit=1)[0].strip()
                rationale = sentence
        ...
        for reference, category, statement, rationale in anchors:
            heading = f"**{reference}" + (f" — {category}" if category else "") + "**"
            body = f"{heading}\n- {statement}."
            if rationale:
                body += f"\n- **Rationale:** {rationale}"
            pieces.append(body)
    ...
    def _structured_summary_from_context(self, context: str, reference, display_name: str):
        """Return a concise exact Rule/Directive summary without raw code leakage."""
        ...
        category = self._structured_labeled_block_from_context(
            context, "Category", stop_on_section_heading=False
        )
        category = re.sub(r"\s+", " ", category).strip() if category else ""
        rationale = self._structured_rationale_from_context(context)
        rationale = re.sub(r"\s+", " ", rationale).strip() if rationale else ""
        if rationale:
            rationale = re.split(r"(?<=[.!?])\s+", rationale, maxsplit=1)[0].strip()

        first = f"**{display_name}" + (f" — {category}" if category else "") + "**"
        pieces = [first, statement.rstrip(".") + "."]
        if rationale:
    ...
    def _is_grounded_followup_candidate(question: str, state) -> bool:
        ...
        followup_signal = re.search(
            r"^(?:why|bakit|how\s+do\s+i|how\s+can\s+i|paano|"
            ...
            r"explain\b.*\b(?:why|bakit)\b|may\s+(?:example|halimbawa)\b|"
            r"ano(?:ng)?\s+(?:rule|directive)\b|what\s+(?:rule|directive)\b|"
        ...
        # Short requests for an example/detail/requirement are frequently
        # natural continuations of the immediately grounded source. Keep this
        # conservative: require a compact utterance and one clearly relational
        # noun, while leaving self-contained new subjects on normal retrieval.
        if len(clean.split()) <= 7 and re.search(
...
"""
    ...
    def _structured_explanation_from_context(
        ...
        """Render one exact Rule/Directive in the source's own section order.

        v6.4.32 makes exact MISRA explanations source-driven rather than a
        fixed field template.  Every recognized field that actually exists in
        the retrieved Rule/Directive block is retained in source order.
        Examples keep code blocks; bullets/numbering remain structured.
        """
        ...
        pieces = [f"{display_name}: {statement.rstrip('.')} .".replace(" .", ".")]

        label_pattern = re.compile(
            r"(?im)^\s*(Category|Analysis|Applies\s+to|Amplification|Rationale|"
            r"Example|Examples|Exception|Exceptions|Note|Notes|See\s+also)\s*:?\s*$"
        )
        source_labels = []
        seen = set()
        for match in label_pattern.finditer(str(context or "")):
            raw_label = re.sub(r"\s+", " ", match.group(1)).strip()
            folded = raw_label.casefold()
            if folded in {"example", "examples"}:
                canonical = "Examples"
            elif folded in {"exception", "exceptions"}:
                canonical = "Exceptions"
            elif folded in {"note", "notes"}:
                canonical = "Notes"
            elif folded == "applies to":
                canonical = "Applies to"
            elif folded == "see also":
                canonical = "See also"
            else:
                canonical = raw_label
            ...
            source_labels.append((canonical, raw_label))

        for canonical, raw_label in source_labels:
            lookup_labels = [raw_label]
            if canonical == "Examples":
                lookup_labels = ["Examples", "Example"]
            elif canonical == "Exceptions":
                lookup_labels = ["Exceptions", "Exception"]
            elif canonical == "Notes":
                lookup_labels = ["Notes", "Note"]

            value = ""
            source_lookup_label = lookup_labels[0]
            preserve_layout = canonical in {"Examples", "Amplification", "Rationale", "Exceptions", "Notes", "See also"}
            for lookup_label in lookup_labels:
                ...
                if value:
                    source_lookup_label = lookup_label
                    break
            if not value:
                continue

            if canonical in {"Category", "Analysis", "Applies to"}:
                normalized = re.sub(r"\s+", " ", value).strip()
                pieces.append(f"{canonical}\n{normalized}")
                continue

            if canonical == "Examples":
                rendered = self._format_structured_example_answer(
                    value,
                    display_label=("Examples" if source_lookup_label.casefold() == "examples" else "Example"),
                )
                if rendered:
                    pieces.append(rendered)
                continue

            formatted = self._format_structured_prose_block(value)
            if formatted:
                pieces.append(f"{canonical}:\n{formatted}")
    ...
    def _apply_structured_answer_focus(
        ...
        if answer_focus.startswith("STRUCTURED SIMPLE EXPLANATION:"):
            ...
            answer = f"**{display_name}:** {statement.rstrip('.')}."
            if source_explanation:
        ...
        if answer_focus.startswith("STRUCTURED EXPLANATION WITH CATEGORY:"):
            ...
            if rationale:
                rationale = re.sub(r"^\d+\.\s*", "", rationale).strip()
                rationale = re.split(r"(?<=[.!?])\s+", rationale, maxsplit=1)[0].strip()

            heading = f"**{display_name}" + (f" — {category}" if category else "") + "**"
            parts = [heading, statement.rstrip(".") + "."]
            if rationale:
        ...
        if answer_focus.startswith("STRUCTURED SUMMARY:"):
            ...
            return summary or f"{display_name}: {statement}"
        ...
        if answer_focus.startswith("STRUCTURED EXPLANATION WITH EXAMPLE:"):
            explanation = self._structured_explanation_from_context(
                ...
                reference=reference,
                display_name=display_name,
            ) or f"{display_name}: {statement}"
            example = self._structured_labeled_block_from_context(
                context,
                "Examples",
                preserve_layout=True,
                stop_on_section_heading=False,
            ) or self._structured_labeled_block_from_context(
                context,
                "Example",
                preserve_layout=True,
                stop_on_section_heading=False,
    ...
    def _format_answer_presentation(
        ...
        # A single numbered/bulleted model response is not a list and looks
        # awkward in the UI. Keep only its factual text.
...
"""
    ...
    def _retrieve_context_for_request(
        ...
        if not misra_compliance_mode:
            return self.query_service.retrieve_context(
                search_question,
        ...
        try:
            ...
            if exact_evidence:
                ...
                return (
                    MisraComplianceMode.build_grounded_context(exact_evidence, semantic_target_question),
                    exact_evidence,
                )
    ...
    def ask(
        ...
        precomputed_structured_multi_answer = (
            self._deterministic_structured_multi_reference_answer(results)
        ...
        if precomputed_structured_multi_answer:
