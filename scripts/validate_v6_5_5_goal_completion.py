...
manifest = json.loads(read("FINAL_COMPONENT_MANIFEST_v6.5.5.4.json"))
...
maintenance_match = re.search(r"\.st-key-kb_maintenance_panel\s*\{(?P<body>.*?)\}", styles, re.S)
check("Maintenance panel remains in normal flow", bool(maintenance_match) and "position: fixed" not in maintenance_match.group("body"))
