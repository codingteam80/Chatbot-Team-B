...
from retrieval.multi_query import generate_multi_query_variants
...
class CompanyRetriever:
    ...
    def __init__(self):
        ...
        self.reranker = None
        self.semantic_rule_resolver = None
        # Process-local cache shared by the Rule-level semantic fallback and
        # the established chunk-level MultiQuery path.  It prevents generating
        # the same two variants twice for one semantic target.
        self._multi_query_search_cache = {}
        # Cache the two generated alternatives by semantic intent, independent
        # of the decorated search query.  v6.5.0 keyed the whole search string,
        # so the Rule-level pass and later chunk fallback could generate the
        # same alternatives twice (the latter adds a ``MISRA C | ...`` prefix).
        self._multi_query_variant_cache = {}
    ...
    def _get_semantic_rule_resolver(self):
        """Return the corpus-derived MISRA Rule-level semantic resolver.

        v6.5.0 keeps this lazy so non-MISRA sessions pay no new cost.  Startup
        prewarm normally builds/loads the small derived index ahead of the first
        question; when prewarm is disabled, the same cache is built on demand.
        """

        if self.semantic_rule_resolver is None:
            try:
                from runtime.prewarm import wait_for_retrieval_prewarm
                wait_for_retrieval_prewarm(timeout=90.0)
            except Exception:
            ...
            from retrieval.semantic_rule_resolver import SemanticRuleResolver

            self.semantic_rule_resolver = SemanticRuleResolver(
                embedding_model=get_embedding_model(),
                reranker=self._get_reranker(),
            )
        return self.semantic_rule_resolver

    def resolve_misra_rule_semantics(self, query):
        """Resolve natural MISRA language against corpus-derived Rule profiles.

        v6.5.0 first tries the original query alone.  If the Rule-level result
        is ambiguous, the existing structured MultiQuery generator supplies
        exactly two intent-preserving variants and the resolver fuses Rule
        profile ranks with RRF before the existing BGE precision check.  The
        variants are cached so the later chunk-level fallback reuses them
        ...
        resolver = self._get_semantic_rule_resolver()
        resolver_ready_seconds = time.perf_counter() - resolver_started

        single_started = time.perf_counter()
        result = resolver.resolve(clean_query)
        single_query_seconds = time.perf_counter() - single_started
        single_query_snapshot = {
            "status": str(result.get("status", "")),
            "accepted": bool(result.get("accepted")),
            "reference": str(result.get("reference", "")),
            "acceptance_basis": str(result.get("acceptance_basis", "")),
            "similarity_margin": result.get("similarity_margin"),
            "bge_top_reference": str(result.get("bge_top_reference", "")),
            "bge_top_score": result.get("bge_top_score"),
        }

        multi_query_generation_seconds = 0.0
        widened_resolution_seconds = 0.0
        latency_decision = (
            "single_query_accepted"
            if result.get("accepted")
            else "single_query_not_accepted"
        )

        if (
            not result.get("accepted")
            and str(result.get("status", "")).casefold() not in {"unavailable", "no_query"}
            and MULTI_QUERY_RETRIEVAL_ENABLED
            and clean_query
        ):
            mq_started = time.perf_counter()
            searches = self._generate_multi_query_searches(
                clean_query,
                intent_query=clean_query,
                structured_reference=None,
            )
            multi_query_generation_seconds = time.perf_counter() - mq_started
            if len(searches) > 1:
                first_status = str(result.get("status", ""))
                widened_started = time.perf_counter()
                widened = resolver.resolve(
                    clean_query,
                    query_variants=searches[1:],
                )
                widened_resolution_seconds = time.perf_counter() - widened_started
                widened["multi_query_rule_resolution"] = True
                widened["multi_query_queries"] = list(searches)
                widened["single_query_status"] = first_status
                result = widened
                latency_decision = "multi_query_used_for_ambiguous_single_query"
            else:
                latency_decision = "multi_query_generation_returned_no_safe_variants"
        elif result.get("accepted"):
            latency_decision = "multi_query_skipped_single_query_proven"

        total_seconds = time.perf_counter() - started
        result["latency_profile"] = {
            "resolver_ready_seconds": round(resolver_ready_seconds, 4),
            "single_query_seconds": round(single_query_seconds, 4),
            "multi_query_generation_seconds": round(multi_query_generation_seconds, 4),
            "widened_resolution_seconds": round(widened_resolution_seconds, 4),
            "total_seconds": round(total_seconds, 4),
            "decision": latency_decision,
            "single_query": single_query_snapshot,
        }

        evidence_logger.record_event(
            event_name="MISRA SEMANTIC LATENCY DECISION",
            status=latency_decision.upper(),
            details=result["latency_profile"],
        )

        evidence_logger.record_event(
            event_name="MISRA CORPUS SEMANTIC RULE RESOLUTION",
            status=str(result.get("status", "unknown")).upper(),
            details={
                "accepted": bool(result.get("accepted")),
                "reference": str(result.get("reference", "")),
                "candidates": list(result.get("candidates") or []),
                "query_count": int(result.get("query_count", 1) or 1),
                "multi_query_rule_resolution": bool(result.get("multi_query_rule_resolution")),
                "fusion": str(result.get("multi_query_fusion", "single-query cosine")),
                "acceptance_basis": str(result.get("acceptance_basis", "")),
                "similarity_margin": result.get("similarity_margin"),
                "bge_top_reference": str(result.get("bge_top_reference", "")),
                "bge_top_score": result.get("bge_top_score"),
                "full_multi_query_consensus": result.get("full_multi_query_consensus"),
                "seconds": round(total_seconds, 4),
                "latency_decision": latency_decision,
                "policy": (
                    "Corpus-derived Rule profiles first; ambiguous single-query "
                    "results widen recall with original + two MultiQuery variants "
                    "at Rule level before chunk-level retrieval."
                ),
    ...
    def _generate_multi_query_searches(self, query, intent_query=None, structured_reference=None):
        ...
        Exact structured requests are deliberately excluded.  This helper runs
        only after deterministic/BM25 fast paths have failed, so MultiQuery
        improves semantic recall without slowing the common exact-rule path.
        """

        original = re.sub(r"\s+", " ", str(query or "")).strip()
        if not original or not MULTI_QUERY_RETRIEVAL_ENABLED or structured_reference:
            return [original] if original else []

        prompt_question = re.sub(
        ...
        intent_cache_key = prompt_question.casefold()

        # First prefer intent-only variant reuse.  The original search string
        # may be decorated differently by the semantic and chunk-level paths,
        # but both represent the same user intent and must reuse the same two
        # generated alternatives.
        cached_variants = self._multi_query_variant_cache.get(intent_cache_key)
        if cached_variants:
            searches = [original]
            ...
            evidence_logger.record_event(
                ...
                status="REUSED CACHED VARIANTS",
                details={
                    "original_query": original,
                    "intent_query": prompt_question,
                    "alternative_queries": searches[1:],
                    "total_queries": len(searches),
                    "cache_scope": "semantic intent",
                    "fusion": "reciprocal-rank fusion; no duplicate variant-generation LLM call",
                },
            )
            return searches

        # Retain the original exact-search cache as a compatibility fallback.
        cached = self._multi_query_search_cache.get(cache_key)
        if cached:
            searches = list(cached)
            evidence_logger.record_event(
                event_name="MULTI-QUERY RETRIEVAL",
                status="REUSED CACHED VARIANTS",
                details={
                    ...
                    "alternative_queries": searches[1:],
                    "total_queries": len(searches),
                    "cache_scope": "exact search query",
                    "fusion": "reciprocal-rank fusion; no duplicate variant-generation LLM call",
                },
            )
            return searches
        ...
        try:
            ...
            variants = generate_multi_query_variants(
                OllamaClient(OLLAMA_FAST_MODEL),
                prompt_question,
            )
        except Exception as error:
            evidence_logger.record_event(
                event_name="MULTI-QUERY RETRIEVAL",
                status="VARIANT GENERATION SKIPPED",
                details={
                    "error": str(error),
                    "seconds": round(time.perf_counter() - started, 4),
        ...
        if len(searches) > 1:
            # Small bounded process-local caches: one preserves historical
            # exact-search behavior; the intent-only cache is what bridges the
            # semantic Rule pass to a decorated chunk-level fallback.
            ...
            variants_only = tuple(searches[1:])
            ...
            self._multi_query_variant_cache[intent_cache_key] = variants_only

        evidence_logger.record_event(
            event_name="MULTI-QUERY RETRIEVAL",
            status="ENABLED" if len(searches) > 1 else "NO SAFE VARIANTS",
            details={
                "original_query": original,
                "intent_query": prompt_question,
                "alternative_queries": searches[1:],
    ...
    def _multi_query_rrf_fuse(self, result_sets):
        """Fuse one retrieval channel across query rewrites with RRF.

        RRF uses only ranks, making BM25 results from differently worded queries
        comparable without pretending their raw scores share one scale.  The
        existing HybridRetriever and BGE reranker still decide final relevance.
        """

        sets = [list(items or []) for items in (result_sets or []) if items is not None]
        if not sets:
            return []
        if len(sets) == 1:
            return sets[0]
        ...
        for query_index, items in enumerate(sets):
            for rank, item in enumerate(items, start=1):
                metadata = dict(item.get("metadata", {}) or {})
                key = self._chunk_key(metadata)
                if not key:
                    key = (
                        str(metadata.get("file_path") or metadata.get("file_name") or ""),
                ...
                if entry is None:
                    ...
                    entry["_multi_query_rrf_raw"] = 0.0
                    entry["_multi_query_hits"] = 0
                    entry["_multi_query_query_indexes"] = []
                    entry["_multi_query_best_raw_score"] = float(item.get("score", 0.0) or 0.0)
                    fused[key] = entry
                entry["_multi_query_rrf_raw"] += 1.0 / (rrf_k + float(rank))
                entry["_multi_query_hits"] += 1
                entry["_multi_query_query_indexes"].append(query_index)
                entry["_multi_query_best_raw_score"] = max(
                    float(entry.get("_multi_query_best_raw_score", 0.0) or 0.0),
                    float(item.get("score", 0.0) or 0.0),
                )

        max_rrf = max(
            (float(item.get("_multi_query_rrf_raw", 0.0) or 0.0) for item in fused.values()),
            default=1.0,
        ) or 1.0
        output = []
        for item in fused.values():
            normalized_rrf = float(item.get("_multi_query_rrf_raw", 0.0) or 0.0) / max_rrf
            item["_multi_query_rrf_score"] = normalized_rrf
            item["score"] = normalized_rrf
            output.append(item)

        output.sort(
            key=lambda item: (
                float(item.get("_multi_query_rrf_score", 0.0) or 0.0),
                int(item.get("_multi_query_hits", 0) or 0),
                float(item.get("_multi_query_best_raw_score", 0.0) or 0.0),
            ),
            reverse=True,
        )
        return output

    def _near_threshold_structured_rule_rescue(self, query, candidates, slots):
        """Rescue a directly matching Rule/Directive body just below 0.55.

        This does not lower the global confidence threshold.  It is limited to
        authoritative requirement bodies with strong lexical overlap and a
        reranker score within a narrow margin of the configured gate.
        """
        ...
        query_tokens = {
            ...
            for token in re.findall(r"[a-z0-9]+", self._normalize_text(query))
        }
        query_tokens -= {
            "what", "which", "are", "is", "the", "for", "about", "apply",
            "applies", "applicable", "rule", "rules", "misra", "restriction",
            "restrictions", "requirement", "requirements", "does", "do", "can",
        }
        query_tokens = {token for token in query_tokens if token and len(token) >= 3}
        if not query_tokens:
            return []

        floor = max(0.0, float(MIN_RETRIEVAL_SCORE) - 0.06)
        rescued = []
        for item in candidates or []:
            score = float(item.get("rerank_score", 0.0) or 0.0)
            if score < floor or score >= float(MIN_RETRIEVAL_SCORE):
                continue
            metadata = item.get("metadata", {}) or {}
            text = str(item.get("text", "") or "").strip()
            section_type = str(metadata.get("section_type", "") or "").casefold()
            section_role = str(metadata.get("section_role", "") or "").casefold()
            structured_body = bool(
                section_type in {"rule", "directive"}
                or re.match(r"^(?:Rule|Directive)\s+\d+(?:\.\d+)+\b", text, re.I)
            )
            if not structured_body:
                continue
            if section_role and section_role not in {"requirement", "statement"}:
                continue
            ...
            statement = " ".join(statement_parts) if statement_parts else text
            statement_tokens = {
                self._structured_catalog_token(token)
                for token in re.findall(r"[a-z0-9]+", self._normalize_text(statement))
            }
            overlap = query_tokens.intersection(statement_tokens)
            required = 1 if len(query_tokens) == 1 else 2
            multi_hits = int(item.get("_multi_query_hits", 0) or 0)
            if len(overlap) < required and multi_hits < 2:
                continue

            item["_structured_near_threshold_rescue"] = True
            item["_structured_near_threshold_original_score"] = score
            rescued.append(item)
            if len(rescued) >= slots:
                break

        if rescued:
            evidence_logger.record_event(
                event_name="STRUCTURED NEAR-THRESHOLD RESCUE",
                status="ACCEPTED",
                details={
                    "global_threshold_unchanged": float(MIN_RETRIEVAL_SCORE),
                    "rescue_floor": floor,
                    "references": [
                        str((item.get("metadata", {}) or {}).get("section_title", ""))
                        or str((item.get("metadata", {}) or {}).get("rule_id", ""))
                        for item in rescued
                    ],
                    "scores": [
                        float(item.get("_structured_near_threshold_original_score", 0.0) or 0.0)
                        for item in rescued
                    ],
                },
            )
    ...
    def _retrieve_exact_structured_reference(
        ...
        # If an indexed Rule/Directive requirement was split at a visually
        # prominent continuation before its Category row, recover that exact
        # authoritative source block from the PDF instead of returning a
        # truncated exact lookup. This is identifier-driven only after the user
        # explicitly requested the reference; no natural-language phrase maps
        # to a Rule here.
        if reference.kind == "rule":
            has_complete_category = any(
                re.search(r"(?im)^\s*Category\s*$", str(record.get("text", "") or ""))
                for record in seed_records
            )
            if not has_complete_category:
                template = dict((seed_records[0].get("metadata", {}) or {}))
                major = str(reference.identifier or "").split(".", 1)[0]
                for source_candidate in self._authoritative_rule_source_candidates(template):
                    recovered = self._source_pdf_rule_family_supplement(
                    ...
                    for item in recovered:
                        recovered_id = str(
                            (item.get("metadata", {}) or {}).get("rule_id", "") or ""
                        ).strip()
                        if recovered_id == str(reference.identifier or "").strip():
                            clone = dict(item)
                            clone["_exact_structured_match"] = True
                            clone["metadata"] = dict(clone.get("metadata", {}) or {})
                            clone["metadata"].update({
                                "exact_structured_match": True,
                                "exact_reference": reference.display_name,
                                "exact_group_chunks": 1,
                            })
                            return [clone]
    ...
    def _retrieve_structured_rule_major_family(self, major_rule_id):
        """Return one grounded summary record for an exact major Rule family.

        A user may naturally say ``Rule 17`` even though the authoritative
        inventory contains only child identifiers such as ``Rule 17.1``.
        Treat that as an incomplete family reference, not as a missing document
        and not as permission to substitute a neighboring Rule semantically.
        """

        major = str(major_rule_id or "").strip()
        if not re.fullmatch(r"\d+", major):
            return []
        ...
        for record in records:
            ...
            if (
                ...
                or str(metadata.get("section_role", "") or "").casefold() not in {"", "requirement"}
                or not pattern.fullmatch(rule_id)
            ):
                continue
            text = str(record.get("text", "") or "").strip()
            if not text or not re.match(rf"^\s*Rule\s+{re.escape(rule_id)}\b", text, re.IGNORECASE):
                continue
            family.append((rule_id, text, metadata))
        ...
        family.sort(key=rule_key)
        ids = [item[0] for item in family]
        first_meta = dict(family[0][2])
        page_values = []
        for _rid, _text, metadata in family:
            for key in ("page_start", "page_end", "page_number", "page"):
        ...
        first_meta.pop("rule_id", None)
        first_meta["section_type"] = "rule_family"
        first_meta["section_title"] = f"Rule {major} family"
        first_meta["exact_reference"] = f"Rules {ids[0]}–{ids[-1]}"
        if page_values:
            first_meta["page_start"] = min(page_values)
            first_meta["page_end"] = max(page_values)

        return [{
            "text": "\n\n".join(text for _rid, text, _meta in family),
            "metadata": first_meta,
            "score": 1.0,
            "rerank_score": 1.0,
            "_structured_major_family_anchor": True,
            "_structured_major_family_id": major,
            "_structured_major_family_members": ids,
        }]
    ...
    def _retrieve_structured_toolchain_configuration_section(
        self,
        query,
        intent_query=None,
    ):
        """Resolve explicit compiler configuration/options wording to source guidance.

        Generic *compiler switch* / *compiler flag* wording is intentionally not
        treated as synonymous with compiler configuration.  The answer layer
        asks for clarification for those broad unnamed terms.  Explicit
        configuration, option, optimization, target, variant, and diagnostic
        wording can still use this deterministic source-grounded Section path.

        C ``switch`` statements and preprocessor ``#if``/``#elif`` rules remain
        separate.  The target Section is discovered from indexed metadata.
        """

        raw_query = str(intent_query or query or "")
        semantic_query = self._normalize_text(raw_query)
        if not semantic_query:
            return []

        # Explicit C-language switch statements belong to the Rule 16 family.
        if re.search(
        ...
        # Explicit preprocessor questions are not compiler-option aliases.
        if re.search(
        ...
        generic_switch_or_flag = bool(
            re.search(r"\b(?:switch|switches|flag|flags)\b", semantic_query)
        )
        ...
        if generic_switch_or_flag and not explicit_configuration:
            return []

        has_toolchain_subject = bool(
            re.search(
                r"\b(?:compiler|toolchain|build(?:\s+tool)?)\b",
                semantic_query,
            )
        )
        has_configuration_term = bool(
            re.search(
                r"\b(?:option|options|setting|settings|configuration|configure|"
                r"configured|optimization|optimisation|diagnostic|diagnostics|"
                r"message|messages|target|targets|variant|variants|c90|c99)\b",
                semantic_query,
            )
        )
        if not (has_toolchain_subject and has_configuration_term):
            return []
        ...
        for record in records:
            ...
            if not re.search(r"\bcompiler\b", title_norm):
                continue
            if not re.search(r"\bconfigur(?:e|ed|ation)\b", title_norm):
                continue

            # Prefer a concise, exact compiler-configuration heading.
            score = 0
            score += 3 if "compiler configuration" in title_norm else 0
            score += 1 if len(re.findall(r"[a-z0-9]+", title_norm)) <= 6 else 0
            candidates.append((score, section_id, title, record))

        if not candidates:
            return []

        candidates.sort(key=lambda row: row[0], reverse=True)
        best_score, best_id, best_title, best_record = candidates[0]
        ...
        # Some structure-aware chunks retain a continuation paragraph under a
        # broader ancestor Section metadata label.  Append only an immediately
        # adjacent source chunk that is still before the next peer subsection.
        # This recovers source continuation text without widening to semantic
        # retrieval or another Rule family.
        seed_metadata = dict(best_record.get("metadata", {}) or {})
        file_key = (
            seed_metadata.get("file_path")
            or seed_metadata.get("file_name")
        )
        try:
            seed_chunk_id = int(seed_metadata.get("chunk_id", -1))
        except (TypeError, ValueError):
            seed_chunk_id = -1

        continuation_text = ""
        continuation_metadata = {}
        if file_key and seed_chunk_id >= 0:
            original_parts = best_id.split(".")
            for chunk_id, record in self._same_file_records_sorted(file_key):
                if chunk_id <= seed_chunk_id:
                    continue
                if chunk_id > seed_chunk_id + 2:
                    break
                ...
                candidate_parts = [
                    part for part in candidate_section_id.split(".") if part
                ]

                # A new peer/deeper numbered subsection means the original
                # compiler-configuration Section has ended.
                if (
                    candidate_section_id
                    and candidate_section_id != best_id
                    and len(candidate_parts) >= len(original_parts)
                    and candidate_parts[: max(0, len(original_parts) - 1)]
                    == original_parts[: max(0, len(original_parts) - 1)]
                ):
                    break

                # Only accept an immediate ancestor-labelled continuation whose
                # body still explicitly discusses the compiler.  Remove the
                # repeated ancestor heading before merging.
                if chunk_id == seed_chunk_id + 1:
                    body_lines = [
                        line.strip()
                    ...
                    if (
                        body
                        and re.search(r"\bcompiler\b", self._normalize_text(body))
                        and (
                            not candidate_parts
                            or len(candidate_parts) < len(original_parts)
                        )
                    ):
        ...
        for item in results:
            ...
            if continuation_text:
                ...
                metadata = dict(item.get("metadata", {}) or {})
                try:
                    metadata["exact_chunk_end"] = max(
                        int(metadata.get("exact_chunk_end", seed_chunk_id)),
                        int(continuation_metadata.get("chunk_id", seed_chunk_id)),
                    )
                except (TypeError, ValueError):
                    pass
                for key in ("page_end", "page_start", "page_number", "page"):
                    value = continuation_metadata.get(key)
                    if isinstance(value, int):
                        metadata["page_end"] = max(
                            int(metadata.get("page_end", value) or value),
                            value,
                        )
                        break
                item["metadata"] = metadata
                item["_structured_section_continuation_recovered"] = True

            item["_structured_section_title_anchor"] = True
            item["_structured_toolchain_configuration_anchor"] = True
            item["_structured_section_title_score"] = best_score
            item["_structured_section_title"] = best_title
            metadata = dict(item.get("metadata", {}) or {})
            metadata["exact_reference"] = reference.display_name
    ...
    def _retrieve_structured_rule_catalog(
        ...
        if category_match:
            # Repair a rare structured-boundary split before category
            # enumeration.  If a Rule requirement chunk lost its Category row
            # (for example because a title-like continuation line was detected
            # as a heading), recover only that Rule from the authoritative PDF
            # text.  This is dynamic/source-grounded and does not encode any
            # Rule identifier.
            incomplete_ids = [
                rule_id
                for rule_id, record in rule_records.items()
                if not re.search(r"(?im)^\s*Category\s*$", record.get("text", ""))
            ]
            for incomplete_id in incomplete_ids:
                current = rule_records.get(incomplete_id) or {}
                metadata_template = dict(current.get("metadata", {}) or {})
                major = incomplete_id.split(".", 1)[0]
                recovered = []
                for source_candidate in self._authoritative_rule_source_candidates(metadata_template):
                    # Prefer the same structure extractor used by ingestion;
                    # the continuation safeguard preserves wrapped requirement
                    # lines before the Category row. Fall back to literal PDF
                    # text recovery for older/unusual layouts.
                    recovered = self._source_pdf_rule_family_supplement(
                        source_path=source_candidate,
                        major_rule_id=major,
                    ...
                    if not recovered:
                        recovered = self._source_pdf_text_rule_family_supplement(
                            source_path=source_candidate,
                            major_rule_id=major,
                            metadata_template=metadata_template,
                        )
                ...
                for item in recovered:
                    ...
                    if rid != incomplete_id:
                        continue
                    recovered_text = str(item.get("text", "") or "").strip()
                    if re.search(r"(?im)^\s*Category\s*$", recovered_text):
                        rule_records[incomplete_id] = {
                            "text": recovered_text,
                            "metadata": dict(item.get("metadata", {}) or metadata_template),
                        }
                        break
            ...
            stop_words = {
                ...
                "relevant", "cover", "covers", "covered", "covering",
                "requirement", "requirements", "restriction", "restrictions",
            }
            # Token normalization performs light stemming (e.g. related ->
            # relat).  Normalize stop words through the same function so generic
            # list language cannot accidentally become a retrieval concept.
            stop = {self._structured_catalog_token(token) for token in stop_words}
            query_tokens = {
                self._structured_catalog_token(token)
            ...
            ambiguous_toolchain_switch_query = bool(
                re.search(
                    r"\b(?:compiler|toolchain|build|command[- ]?line)\b",
                    semantic_query,
                )
                and re.search(r"\b(?:switch|switches|flag|flags)\b", semantic_query)
                and not re.search(
                    r"\b(?:compiler\s+configuration|configuration\s+of\s+the\s+compiler|"
                    r"configure|configured|setup)\b",
                    semantic_query,
            ...
            if ambiguous_toolchain_switch_query:
                # The UI asks for clarification before retrieval.  Keep this
                # lower-level catalog path conservative too so direct callers
                # cannot silently reinterpret a generic compiler switch/flag as
                # either Rule 20.9 or the C switch-statement family.
                return []
            ...
            toolchain_option_query = bool(
                ...
                and re.search(r"\b(?:option|options)\b", semantic_query)
            )
            ...
            # Exact concept intersection has priority over broad family
            # expansion.  A query that combines several distinctive concepts
            # (for example string handling + pointer + bounds) should first
            # select Rule statements whose own requirement text covers ALL of
            # those concepts.  Only when no such statement exists may the
            # broader pointer/Section family logic run.
            distinctive = {
                token for token in query_tokens
                if token not in {
                    "code", "type", "value", "object", "function", "expression",
                    "statement", "requirement", "guidance", "standard",
                }
            }
            if not chosen_ids and len(distinctive) >= 2:
                exact_concept_ids = []
                for rule_id, record in rule_records.items():
                    ...
                    if distinctive.issubset(statement_tokens):
                        exact_concept_ids.append(rule_id)
                if exact_concept_ids:
                    chosen_ids = exact_concept_ids
                    family_label = " ".join(sorted(distinctive))
                    family_heading = f"{family_label.capitalize()}-related rules"
                    family_scope_note = (
                        "Scope: exact concept intersection in authoritative Rule "
            ...
            if broad_pointer_query and not chosen_ids:
                for rule_id, record in rule_records.items():
                    lines = [
                        line.strip()
            ...
            # Before expanding a whole Section 8.x family, check whether the
            # user's extra topic words identify a smaller set of Rule statements.
            # Example pattern: a section title may match "unused", while the
            # additional word "declarations" narrows the request to only the
            # Rule statements that explicitly contain both concepts.  This is
            # source-driven and does not hardcode any Rule identifier.
            specific_statement_ids = []
            distinctive = {
                token for token in query_tokens
                if token not in {
                    "code", "type", "value", "object", "function", "expression",
            ...
            if not chosen_ids and distinctive:
                for rule_id, record in rule_records.items():
                    lines = [line.strip() for line in record["text"].splitlines() if line.strip()]
                    statement_parts = []
                    for line in lines[1:]:
                    ...
                    statement_tokens = {
                        self._structured_catalog_token(token)
                        for token in re.findall(r"[a-z0-9]+", self._normalize_text(" ".join(statement_parts)))
                    }
                    if distinctive.issubset(statement_tokens):
                        specific_statement_ids.append(rule_id)
    ...
    @staticmethod
    def _misra_role_reference(item):
        """Return a structured MISRA reference and semantic role when proven by metadata."""
        ...
        if section_type in {"rule", "directive"}:
            ...
            kind = section_type
        elif parent_type in {"rule", "directive"}:
            identifier = str(
                metadata.get("parent_identifier", "")
            ...
            kind = parent_type
        elif metadata.get("parent_rule_id"):
            kind = "rule"
            identifier = str(metadata.get("parent_rule_id") or "").strip()
        elif metadata.get("parent_directive_id"):
            kind = "directive"
            identifier = str(metadata.get("parent_directive_id") or "").strip()
        else:
            return "", role

        if not identifier:
            return "", role
        label = "Directive" if kind == "directive" else "Rule"
        return f"{label} {identifier}", role

    def _prepare_misra_role_aware_reranker_pool(self, candidates, limit=14):
        """Prefer authoritative Rule-role chunks over appendix/summary noise.

        The v4 corpus intentionally preserves broad document sections, Appendix
        tables, and family summaries.  They are useful for broad catalog/list
        questions, but a natural single-Rule lookup should not let those chunks
        crowd requirement/rationale/example evidence out of the BGE pool.

        This is metadata-role driven and contains no Rule-number or query-phrase
        mapping.  It does not alter stored chunks or the 900/150 splitter.
        """
        ...
        authoritative_roles = {
            "requirement", "statement", "rationale", "amplification",
            "example", "examples", "exception", "exceptions", "note", "notes",
        }
        role_priority = {
            "requirement": 0,
            "statement": 0,
        ...
        for index, item in enumerate(items):
            reference, role = self._misra_role_reference(item)
            if reference and role in authoritative_roles:
                grouped.setdefault(reference, []).append((index, role_priority.get(role, 3), item))
            else:
                broad.append((index, item))

        structured_count = sum(len(values) for values in grouped.values())
        # Do not force a role-only view when the retrieved pool contains too
        # little structured evidence.  In that case the established path keeps
        # all candidates exactly as before.
        if structured_count < 4:
            return items

        group_rows = []
        for reference, rows in grouped.items():
            first_seen = min(index for index, _, _ in rows)
            rows = sorted(rows, key=lambda row: (row[1], row[0]))
            # Keep the requirement plus at most one best supporting role from
            # the same Rule.  This prevents one long Rule from monopolizing the
            # precision pool while still retaining rationale/example support.
            chosen = rows[:2]
            group_rows.append((first_seen, reference, chosen))
        group_rows.sort(key=lambda row: row[0])

        selected = []
        selected_ids = set()
        # Reserve at most two slots for broad document sections.  They remain
        # available as supporting context but can no longer dominate 10/14
        # reranker positions ahead of exact Rule-role evidence.
        structured_limit = max(1, limit - min(2, len(broad)))
        for _, _, rows in group_rows:
        ...
        # If diversity caps left room, fill from remaining structured evidence
        # before returning.  Ordering remains stable relative to the already
        # informative-ranked candidate list.
        if len(selected) < limit:
            for item in items:
        ...
        evidence_logger.record_event(
            event_name="MISRA ROLE-AWARE PRECISION POOL",
            status="APPLIED",
            details={
                "input_candidates": len(items),
                "structured_role_candidates": structured_count,
                "broad_document_candidates": len(broad),
                "output_candidates": len(selected),
                "references": [
                    reference
                    for _, reference, _ in group_rows[: min(8, len(group_rows))]
                ],
                "policy": (
                    "Requirement/rationale/amplification/example metadata is preferred "
                    "for natural MISRA precision reranking; broad appendix/summary "
                    "sections remain secondary support. Stored chunks are unchanged."
                ),
            },
        )
        return selected
    ...
    def _apply_diversity_filter(
        ...
        for item in results:
            ...
            # Evidence explicitly rescued because it strongly matches the
            # current Rule/Directive intent must survive the generic per-file
            # diversity cap.  The final Top-K remains unchanged; this only
            # prevents a valid near-threshold Rule body from being rescued and
            # then immediately dropped because two neighboring chunks from the
            # same MISRA PDF appeared first.
            pinned_evidence = bool(
                item.get("_structured_near_threshold_rescue")
                or item.get("_misra_rule_body_rescue")
                or item.get("_misra_exact_reference_anchor")
                or item.get("_exact_structured_match")
            )

            if current_count >= max_chunks_per_file and not pinned_evidence:
                continue
    ...
    def retrieve(
        ...
        if structured_reference:
            ...
            # A bare major Rule identifier such as ``Rule 17`` is an
            # incomplete family reference when the authoritative inventory has
            # child Rules (17.1, 17.2, ...). Return one compact grounded family
            # anchor so the answer layer can ask for the exact child Rule.
            if (
                structured_reference.kind == "rule"
                and re.fullmatch(r"\d+", str(structured_reference.identifier or ""))
            ):
                family_results = self._retrieve_structured_rule_major_family(
                    structured_reference.identifier
                )
                family_results = self._filter_source_family(
                    family_results, source_family
                )
                if family_results:
                    self._record_qa_stage(
                        "STRUCTURED RULE FAMILY CLARIFICATION",
                        family_results,
                        limit=len(family_results),
                        accepted=True,
                    )
                    evidence_logger.record_event(
                        event_name="STRUCTURED RULE FAMILY CLARIFICATION",
                        status="MATCHED FAMILY",
                        details={
                            "requested": structured_reference.display_name,
                            "members": family_results[0].get(
                                "_structured_major_family_members", []
                            ),
                        },
                    )
                    evidence_logger.record_retrieval_summary(
                        ...
                        hybrid_candidates=0,
                        reranker_candidates=0,
                        accepted_chunks=len(family_results),
                        rejected_chunks=0,
                        final_chunks=len(family_results),
                        configured_top_k=len(family_results),
                        confidence_threshold=None,
                    )
        ...
        toolchain_configuration_results = (
            self._retrieve_structured_toolchain_configuration_section(
                query=query,
                intent_query=intent_query,
            )
        )
        toolchain_configuration_results = self._filter_source_family(
            toolchain_configuration_results,
            source_family,
        )
        if toolchain_configuration_results:
            self._record_qa_stage(
                "STRUCTURED TOOLCHAIN CONFIGURATION",
                toolchain_configuration_results,
                limit=len(toolchain_configuration_results),
                accepted=True,
            )
            evidence_logger.record_event(
                event_name="STRUCTURED TOOLCHAIN CONFIGURATION RETRIEVAL",
                status="MATCHED SECTION",
                details={
                    "reference": str(
                        (
                            toolchain_configuration_results[0].get(
                                "metadata", {}
                            )
                            or {}
                        ).get("exact_reference", "")
                    ...
                    "continuation_recovered": bool(
                        toolchain_configuration_results[0].get(
                            "_structured_section_continuation_recovered"
                        )
                    ),
                },
            )
            evidence_logger.record_retrieval_summary(
                ...
                hybrid_candidates=0,
                reranker_candidates=0,
                accepted_chunks=len(toolchain_configuration_results),
                rejected_chunks=0,
                final_chunks=len(toolchain_configuration_results),
                configured_top_k=len(toolchain_configuration_results),
                confidence_threshold=None,
            )
            return toolchain_configuration_results
        ...
        # Run the original lexical query first so exact BM25 fast paths and
        # identity OOD rejection remain low-latency and unchanged. MultiQuery is
        # activated only later when full semantic retrieval is actually needed.
        bm25_results = self.bm25.search(
            query,
            bm25_top_k
        )
        ...
        multi_query_searches = self._generate_multi_query_searches(
            query=query,
            intent_query=intent_query,
            structured_reference=structured_reference,
        )
        multi_query_active = len(multi_query_searches) > 1

        if multi_query_active:
            bm25_result_sets = [bm25_results]
            for alternative_query in multi_query_searches[1:]:
                alternative_results = self.bm25.search(
                    alternative_query,
                    bm25_top_k,
                )
                bm25_result_sets.append(
                    self._filter_source_family(alternative_results, source_family)
                )
            bm25_results = self._multi_query_rrf_fuse(bm25_result_sets)

        vector_result_sets = []
        vector_searcher = self._get_vector_searcher()
        for retrieval_query in multi_query_searches or [query]:
            channel_results = vector_searcher.search(
                retrieval_query,
                vector_top_k,
            )
            vector_result_sets.append(
                self._filter_source_family(channel_results, source_family)
            )
        vector_results = self._multi_query_rrf_fuse(vector_result_sets)
        ...
        # v6.5.2: for natural source-scoped MISRA questions, keep broad
        # appendix/summary sections as secondary support instead of allowing
        # them to crowd exact Rule-role chunks out of the reranker pool.
        if (
            str(source_family or "").casefold() == "misra"
            and not is_list_mode
            and not structured_reference
        ):
            candidates = self._prepare_misra_role_aware_reranker_pool(
                candidates,
                limit=14 if multi_query_active else 10,
            )
        ...
        if is_list_mode:
            ...
            # MultiQuery increases recall by adding alternative lexical/vector
            # candidates. Keep a modestly wider reranker pool only when it is
            # active; the existing final Top-K and 0.55 gate remain unchanged.
            candidates = candidates[:14 if multi_query_active else 10]
        ...
        if reranker is not None:
            ...
            rejected_for_rescue = [
                item for item in pre_confidence_ranked
                if item not in ranked
            ]
            ranked.extend(
                self._near_threshold_structured_rule_rescue(
                    query=intent_query or query,
                    candidates=rejected_for_rescue,
                    slots=max(0, final_top_k - len(ranked)),
                )
            )
