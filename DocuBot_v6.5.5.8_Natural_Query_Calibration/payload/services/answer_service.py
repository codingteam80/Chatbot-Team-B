...
class AnswerService:
    ...
    def _is_grounded_followup_candidate(question: str, state) -> bool:
        ...
        # A current turn can contain its own antecedent and still use words such
        # as ``that`` (for example: ``A string call may overflow the destination.
        # Which MISRA requirement addresses that risk?``).  If the same turn
        # contains a substantive Rule/Directive lookup description, treat it as
        # self-contained instead of binding ``that`` to the previous answer.
        # This is identifier-free and does not map any concept to a Rule.
        if state.get("misra") is True and MisraComplianceMode.semantic_rule_lookup_intent(question):
            ...
            lookup_stripped = re.sub(
                r"\b(?:this|that|it|its|same|ito|iyan|iyon|yan|yun|nito|niyan|niyon)\b",
                " ",
                lookup_stripped,
                flags=re.IGNORECASE,
            )
...
"""
    ...
    def _retrieve_context_for_request(
        ...
        # v6.5.5.8 calibration: deterministic source-language cues are already
        # stricter than a generic semantic guess. Resolve those cues directly
        # against authoritative Rule/Directive bodies before paying for the
        # Rule-level embedding + MultiQuery path. This is still corpus-grounded:
        # cues contain requirement wording, never a Rule-number mapping.
        pre_semantic_cues = MisraComplianceMode.semantic_cues(semantic_target_question)
        if pre_semantic_cues:
            try:
                pre_semantic_rescued, pre_semantic_details = (
                    MisraComplianceMode.rule_body_cue_rescue_from_authoritative_corpus(
                        question=semantic_target_question,
                        top_k=6,
                    )
                )
            except Exception as pre_semantic_error:
                pre_semantic_rescued = []
                pre_semantic_details = {"error": str(pre_semantic_error)}

            if pre_semantic_rescued:
                evidence_logger.record_event(
                    event_name="MISRA SOURCE-CUE PRE-SEMANTIC RESCUE",
                    status="MATCHED AUTHORITATIVE CORPUS",
                    details={
                        **pre_semantic_details,
                        "reason": (
                            "Deterministic source-language concepts matched actual "
                            "Rule/Directive bodies before semantic MultiQuery."
                        ),
                    },
                )
                return (
                    MisraComplianceMode.build_grounded_context(
                        pre_semantic_rescued, semantic_target_question
                    ),
                    pre_semantic_rescued,
                )
    ...
    def _post_retrieval_misra_authoritative_promotion(
        ...
        # Preserve explicit structured inventory/comparison intent.  These
        # results are already a complete source-grounded set; collapsing them
        # to one semantically strongest Rule would turn a list request into a
        # single-rule answer.
        if any(
            isinstance(item, dict)
            and (
                item.get("_structured_topic_anchor")
                or item.get("_structured_comparison_anchor")
                or item.get("_structured_major_family_anchor")
            )
            for item in results
        ):
            return "", [], {"preserved_structured_inventory": True}
    ...
    def _explicit_external_knowledge_request(question: str) -> bool:
        ...
        outside_instruction = bool(re.search(
            ...
            r"\b(?:outside\s+(?:the\s+)?(?:company\s+)?(?:knowledge|documents|kb)|"
        ...
        override_language = bool(re.search(
            ...
            r"\b(?:external\s+knowledge|general\s+knowledge|outside\s+knowledge)\b",
            clean,
            re.IGNORECASE,
        ))
        ...
        try:
            ...
            if not answer:
                ...
                if misra_compliance_mode:
                    ...
                    # v6.5.5.7 final answer closure: once retrieval has already
                    # verified a single Rule/Directive, do not let an otherwise
                    # acceptable local-model draft omit that source identity or
                    # drift into an unrelated Unicode script.  The finalizer is
                    # deterministic and can use only the accepted result metadata;
                    # it contains no phrase->Rule mapping.
                    if answer.strip().casefold() != NO_RESULT_MESSAGE.casefold():
                        answer, verified_contract_changes = (
                            enforce_verified_answer_contract(
                                answer=answer,
                                question=final_question,
                                results=results,
                                yes_no_intent=MisraComplianceMode.yes_no_intent(
                                    misra_evidence_question
                                ),
                                require_reference=True,
                            )
                        )
                        if verified_contract_changes:
                            evidence_logger.record_event(
                                event_name="VERIFIED ANSWER CONTRACT",
                                status="APPLIED",
                                details=verified_contract_changes,
                            )
                            verified_answer = answer
                else:
