...
class MisraComplianceMode:
    ...
    @classmethod
    def semantic_rule_resolution_scope(cls, question: str, current_topic: str = "") -> bool:
        """Return True when a text-only turn is safe for corpus Rule resolution.

        v6.5.0 deliberately separates *scope detection* from Rule selection.
        This helper recognizes only that the user is asking a MISRA standards
        question; it never maps words or concepts to a Rule identifier.
        """

        raw = re.sub(r"\s+", " ", str(question or "")).strip()
        if not raw or len(raw) > 1400 or cls.looks_like_c_cpp(raw):
            return False
        lowered = raw.casefold()
        topic = str(current_topic or "").casefold()
        if "misra" not in lowered and "misra" not in topic:
            return False
        if _REF_RE.search(raw):
            return False
        # Broad inventories belong to the structured family/list retriever, not
        # the single-best Rule resolver.  This is intent gating only.
        if re.search(
            r"\b(?:list|enumerate|all|every)\b.{0,40}\b(?:rules?|directives?|guidelines?)\b|"
            r"\b(?:mandatory|required|advisory)\s+(?:misra\s+)?rules\b|"
            r"\brules\s+(?:for|about|related\s+to|applicable\s+to)\b",
            lowered,
        ):
            return False
        return bool(re.search(
            r"\b(?:rule|directive|requirement|guideline|compliant|compliance|"
            r"allowed|permitted|acceptable|valid|violation|violates?|apply|applies|"
            r"applicable|should|shall|must|can|could|may|pwede|puwede|bawal)\b",
            lowered,
        ))
    ...
    def is_request(cls, question: str, current_topic: str = "") -> bool:
        ...
        # v6.4.101: source-information wording such as ``Which MISRA rule
        # covers ...?`` used to be rejected here *before* a recognized natural
        # requirement could reach the authoritative Rule-body path.  Preserve
        # ordinary catalog/reference behavior, but promote a narrow source-
        # backed concept lookup when the turn is explicitly/inherited MISRA and
        # an identifier-free requirement cue is already known.  Final answer
        # shaping still treats it as a requirement lookup rather than a code
        # compliance verdict.
        if cls._EXACT_LOOKUP_ONLY.match(raw):
            return False
        if cls.informational_reference_query(raw):
            if not ((explicit_misra or misra_context) and natural_requirement):
                return False
        if cls.informational_catalog_query(raw):
            if not ((explicit_misra or misra_context) and natural_requirement):
                return False
    ...
    def yes_no_intent(cls, question: str) -> str:
        ...
        # was excluded above as a polite imperative.  v6.4.101 also tolerates
        # a short standards preamble (``In/Under MISRA C, is ... acceptable?``)
        # and placement wording used by scope questions.
        # Inspect both the full turn and its final interrogative sentence.
        # Natural users often describe the scenario first, then ask a trailing
        # question such as ``... . Is that acceptable under MISRA C?``.  The
        # older start-of-string-only check misclassified those turns as plain
        # requirement lookups and skipped the Yes/No relation verifier.
        permission_candidates = [clean]
        sentence_parts = [
            part.strip()
            for part in re.split(r"(?<=[.!?])\s+", clean)
            if part.strip()
        ]
        if sentence_parts and sentence_parts[-1] not in permission_candidates:
            permission_candidates.insert(0, sentence_parts[-1])

        permission_clean = clean
        for permission_candidate in permission_candidates:
            candidate = re.sub(
                r"^(?:in|under)\s+misra(?:\s+c)?\s*,?\s*",
                "",
                permission_candidate,
            )
            if (
                (
                    re.match(r"^(?:does|do|did|is|are|was|were|can|could|should|may)\b", candidate)
                    and re.search(r"\b(?:allow|allows|allowed|permit|permits|permitted|acceptable|valid|okay|ok|use|used|using|have|leave|keep|place|placed|placing|omit|omits|omitted|omitting|exclude|excluded|skip|skipped|remove|removed|jump|jumps|jumping|target|targets|targeting)\b", candidate)
                )
                or re.match(r"^(?:pwede|puwede)\s+ba\b", candidate)
                or re.search(
                    r"\b(?:valid|acceptable|allowed|permitted|pwede|puwede)\s+ba\b",
                    candidate,
                )
            ):
                return "permission"
            # Preserve the historical variable for the conservative modal
            # fallback below; prefer the shortest/question-tail candidate.
            permission_clean = candidate

        # General source-backed modal question.  A sentence such as
        # "May a goto jump to ...?" is standard English permission grammar,
        # not the Tagalog existential "may".  Only promote the modal when the
        # same sentence already resolves to a known natural MISRA requirement
        # cue; this avoids turning arbitrary "may ..." prose into a standards
        # verdict.
        if (
            (
                re.match(r"^(?:can|could)\b", permission_clean)
                or re.match(r"^may\s+(?:a|an|the|this|that|these|those)\b", permission_clean)
            )
            and cls._natural_requirement_cues(question)
        ):
            return "permission"
    ...
    def deterministic_yes_no_answer(
        ...
        # v6.5.0: a corpus-semantic Rule match establishes *which source rule*
        # is relevant, not the polarity of an arbitrary natural-language
        # proposition. Most semantic Yes/No relations therefore still use the
        # compact grounded relation verifier. v6.5.3 makes one generic, source-
        # provable exception: asking whether an explicitly required element may
        # be absent ("without X", "no X", "walang X", "lacks X"). That relation
        # is determinable directly from a source statement that says it shall/
        # must have/contain/include/provide X. No Rule-specific mapping is used.
        semantic_resolved = any(
            isinstance(item, Mapping) and item.get("_misra_semantic_resolver") is True
        ...
        semantic_absence_request = bool(re.search(
            r"\b(?:omit|omits|omitted|omitting|without|leave\s+out|left\s+out|"
            r"exclude|excluded|missing|absent|lacks?|walang)\b|"
            r"\bno\s+[a-z_][a-z0-9_-]*\b",
            re.sub(r"\s+", " ", str(question or "").casefold()),
        ))
        if semantic_resolved and not (
            intent in {"permission", "compliance"}
            and semantic_absence_request
            and not cls.looks_like_c_cpp(question)
        ):
            return ""
        ...
        # v6.4.101: directional scope permission cannot be inferred from the
        # generic positive/prohibitive polarity of Rule 15.3.  A child/nested
        # block is *inside* the goto's block, while the Rule permits only the
        # same block or a block enclosing the goto.  Resolve that relation only
        # when authoritative Rule 15.3 is actually among the accepted evidence.
        if intent == "permission" and not cls.looks_like_c_cpp(question):
            clean_question = re.sub(r"\s+", " ", str(question or "").casefold())
            has_rule_15_3 = any(
                str((item.get("metadata", {}) or {}).get("rule_id", "") or "").strip() == "15.3"
                for item in candidates
            )
            if has_rule_15_3 and re.search(r"\bgoto\b", clean_question):
                asks_inner_target = bool(re.search(
                    r"\b(?:child|nested|inner)\s+block\b|"
                    r"\bnasa\s+(?:isang\s+)?nested\s+block\b|"
                    r"\bsa\s+loob\s+ng\s+(?:pinanggalingang|current|same)\s+block\b",
                    clean_question,
                ))
                asks_allowed_scope = bool(re.search(
                    r"\b(?:same\s+block|enclosing\s+block|outer\s+block)\b",
                    clean_question,
                ))
                rule_statement = next(
                    (statement for item, statement in zip(candidates, statements)
                     if str((item.get("metadata", {}) or {}).get("rule_id", "") or "").strip() == "15.3" and statement),
                    "",
                )
                if asks_inner_target and rule_statement:
                    answer = f"No. Rule 15.3 states that {rule_statement[:1].lower() + rule_statement[1:].rstrip(' .')}."
                    return answer if cls.references_are_grounded(answer, candidates) else ""
                if asks_allowed_scope and rule_statement:
                    answer = f"Yes. Rule 15.3 states that {rule_statement[:1].lower() + rule_statement[1:].rstrip(' .')}."
                    return answer if cls.references_are_grounded(answer, candidates) else ""
        ...
        if intent in {"permission", "compliance"} and absence_request:
        ...
        if intent in {"permission", "compliance"} and absence_request and required_presence_statement:
            reference = ""
            for item in candidates:
    ...
    def _natural_requirement_cues(cls, question: str) -> list[str]:
        ...
        # Linkage/visibility described without a Rule number.  The cue is the
        # authoritative requirement wording, not a query->Rule mapping.
        if (
            (
                re.search(r"\btranslation\s+unit\b", clean)
                and re.search(r"\b(?:function|object|identifier|helper)\b", clean)
                and re.search(r"\b(?:linkage|visible|visibility|external|internal|source\s+file)\b", clean)
            )
            or (
                re.search(r"\b(?:globally|global|externally)\s+visible\b", clean)
                and re.search(r"\b(?:function|object|helper)\b", clean)
                and (
                    re.search(r"\b(?:own|same|single|one)\b.{0,35}\b(?:source\s+file|file|translation\s+unit|\.c\s+file)\b", clean)
        ...
        # Composite arithmetic calculated in a narrower essential type and only
        # then stored in a wider destination.  Require both an operation and a
        # widening/assignment concept so ordinary scalar conversions remain on
        # the Rule 10.3 path.
        width_values = [
            int(value)
        ...
        if explicit_widening and composite_operation and assignment_target:
            add("the value of a composite expression shall not be assigned to an object with wider essential type")

        # Text-only control-expression questions.  Mentioning an integer/scalar
        # or pointer used directly as an if/loop condition is enough to retrieve
        ...
        control_operand = bool(
            re.search(r"\b(?:integer|scalar|pointer)\b", clean)
            or re.search(r"\b(?:if|while)\s*\(\s*[a-z_]\w*\s*\)", clean)
        )
        if control_context and control_operand:
        ...
        # Unused function parameters.  v6.4.100 uses a concept-level lexical
        # relation matcher rather than a growing list of whole-sentence regexes.
        # This lets unseen English/Tagalog/Taglish paraphrases converge before
        # MultiQuery/reranking, while the authoritative corpus still decides the
        # actual Rule identifier.
        if cls._unused_function_parameter_concept(raw):
        ...
        if (
            ...
            and (
                ...
                or re.search(r"\b(?:target|destination)\b", clean)
            )
            and re.search(
                r"\b(?:scope|block|same|child|nested|inner|outer|enclos|restrict|where|location|place|placed|declare|declared)\w*\b",
                clean,
            )
        ):
    ...
    @classmethod
    def _unused_function_parameter_concept(cls, question: str) -> bool:
        """Recognize the *concept* of an unused function parameter.

        v6.4.100 deliberately avoids matching whole test sentences.  Instead it
        looks for a parameter/argument entity and an unused/not-referenced
        relation within a small lexical window.  This handles English, Tagalog,
        and Taglish fillers (for example ``that the function``, ``naman``,
        ``yung``) while keeping unrelated constructs such as macro parameters
        out of the Rule 2.7 path.
        """

        raw = re.sub(r"[-_/]+", " ", str(question or "")).casefold()
        ...
        entity_terms = {"parameter", "parameters", "argument", "arguments"}
        entity_positions = [i for i, token in enumerate(tokens) if token in entity_terms]
        if not entity_positions:
            return False

        direct_unused_terms = {"unused", "unreferenced", "ignored", "neglected"}
        negation_terms = {"never", "not", "without", "hindi", "di", "wala", "walang"}

        def usage_term(token: str) -> bool:
            # Treat ordinary data-use verbs as one semantic family.  The
            # negation/proximity checks below still have to be satisfied, so
            # words such as "access" do not independently trigger Rule 2.7.
            # This generalizes beyond one test phrase: a parameter that is not
            # used/read/accessed/referenced/consumed is the same unused-input
            # concept for retrieval purposes.
            return (
                token in {
                    "use", "used", "using",
                    "read", "reads", "reading",
                    ...
                    "gamit", "gamitin", "gumamit", "gumagamit",
                }
                or token.startswith("referenc")
                or token.startswith("ginagamit")
                or token.startswith("nagagamit")
                or token.startswith("nagamit")
                or token.startswith("gumagamit")
                or token.startswith("nirefer")
            )

        for pos in entity_positions:
            # Do not reinterpret a specifically named macro parameter as a
            # function parameter merely because the word "unused" is nearby.
            left_context = tokens[max(0, pos - 3):pos]
            if "macro" in left_context:
                continue

            lo = max(0, pos - 10)
            hi = min(len(tokens), pos + 11)
            window = tokens[lo:hi]

            # Direct lexical concepts: "unused parameter", "parameter ...
            # unused", or "unreferenced argument".
            for idx, token in enumerate(window):
                absolute_idx = lo + idx
                if token not in direct_unused_terms or abs(absolute_idx - pos) > 7:
                    continue
                between = tokens[min(absolute_idx, pos) + 1:max(absolute_idx, pos)]
                # Keep precision when "unused"/"ignored" clearly modifies a
                # different object (for example an unused *return value*) that
                # merely happens to occur near the word parameter.  Check both
                # the words between the concept and the entity and the closest
                # neighboring noun around the concept token itself.
                blockers = {"return", "value", "label", "macro", "tag", "typedef", "variable"}
                if blockers.intersection(between):
                    continue
                nearby_blocker = False
                for blocker_pos, blocker in enumerate(tokens):
                    if blocker not in blockers:
                        continue
                    if abs(blocker_pos - absolute_idx) <= 2 and abs(blocker_pos - absolute_idx) < abs(pos - absolute_idx):
                        nearby_blocker = True
                        break
                if nearby_blocker:
                    continue
            ...
            # Relational concepts: "parameter that the function never
            # references", "parameter not used", "hindi naman nagagamit
            # na parameter", etc.  Filler words are tolerated, but the
            # negation and usage verb must remain close to each other.
            neg_positions = [lo + i for i, token in enumerate(window) if token in negation_terms]
            usage_positions = [lo + i for i, token in enumerate(window) if usage_term(token)]
            for neg_pos in neg_positions:
                for use_pos in usage_positions:
                    if abs(use_pos - neg_pos) <= 7 and max(abs(neg_pos - pos), abs(use_pos - pos)) <= 12:
                        return True
    ...
    def semantic_cues(cls, question: str) -> list[str]:
        ...
        raw = str(question or "")
        code_only = cls._code_only_view(raw)
        code = code_only or raw
        clean = re.sub(r"\s+", " ", raw).casefold()
        cues: list[str] = []
    ...
    def cue_assessment_state(cls, question: str, cue: str) -> tuple[str, str]:
        ...
        if normalized == cls._normalized_phrase(
            ...
            prose = re.sub(r"\s+", " ", raw.casefold())
            empty_else = bool(
                re.search(r"\b(?:empty|blank)\b.{0,24}\belse\b", prose)
                or re.search(r"\belse\b.{0,24}\b(?:empty|blank)\b", prose)
                or re.search(r"\belse\s+block\b.{0,24}\b(?:nothing|no\s+action)\b", prose)
            )
            if empty_else and re.search(r"\belse\s+if\b|\bif[- ]else[- ]if\b", prose):
                return (
                    "violation",
                    "The described terminating else in an if/else-if chain is empty; the source amplification requires that else to contain at least a side effect or a comment.",
                )
            if empty_else and not code_like:
                return (
                    "uncertain",
                    "Rule 15.7 applies to the terminating else of an if/else-if chain. In that case an empty else is not enough: it must contain at least a side effect or a comment. A simple if statement does not require a final else under this rule.",
                )
        ...
        if (
            ...
            prose = re.sub(r"\s+", " ", raw.casefold())
            if re.search(r"\bpointer\b", prose) and re.search(
                r"\b(?:condition|controlling\s+expression|if|while|loop)\b", prose
            ):
                return (
                    "violation",
                    "The question explicitly describes a pointer used directly as a controlling condition; a pointer value is not an essentially Boolean controlling expression.",
                )
            if re.search(r"\b(?:integer|non[- ]?boolean\s+scalar|scalar)\b", prose) and re.search(
                r"\b(?:condition|controlling\s+expression|if|while|loop)\b", prose
            ):
                return (
                    "violation",
                    "The question explicitly describes an integer/non-Boolean scalar used as a controlling condition; the matched requirement requires an essentially Boolean type.",
                )
            return (
                "uncertain",
    ...
    @classmethod
    def authoritative_semantic_reference_evidence(
        cls,
        records: Sequence[Mapping],
        *,
        kind: str,
        ...
        """Materialize one semantic resolver winner from authoritative source.

        The Rule/Directive identity is supplied by the document-derived semantic
        resolver.  This method independently verifies that exact identifier in
        the BM25 source inventory before any evidence can reach answer generation.
        """

        wanted_kind = str(kind or "").strip().casefold()
        wanted_id = str(identifier or "").strip()
        if wanted_kind not in {"rule", "directive"} or not wanted_id or not records:
            return []
        selected = None
        for item in records:
            if not isinstance(item, Mapping) or not cls._is_citable_rule_body_record(item):
                continue
            item_kind, item_id = cls._reference_key(item)
            if item_kind == wanted_kind and item_id == wanted_id:
                selected = item
                break
        if selected is None:
            return []
        ...
        enriched = dict(cls._expand_rule_body_details(records, selected))
        enriched["_misra_semantic_resolver"] = True
        enriched["_misra_rule_body_rescue"] = True
        enriched["_misra_cue_coverage"] = 1.0
        enriched["_misra_assessment_state"] = "uncertain"
        enriched["_misra_observation"] = ""
        enriched["_misra_semantic_diagnostics"] = dict(diagnostics or {})
    ...
    def semantic_rule_lookup_intent(cls, question: str) -> bool:
        """Generic intent detector for a Rule/Directive lookup after semantic resolution.

        Unlike ``semantic_cues`` this contains no concept vocabulary.  It only
        determines whether the user asked for the applicable source requirement
        rather than a binary compliance/permission verdict.
        """

        raw = re.sub(r"\s+", " ", str(question or "")).strip()
        if not raw or cls.looks_like_c_cpp(raw) or cls.yes_no_intent(raw):
            return False
        clean = raw.casefold()
        if re.search(r"\b(?:why|bakit|reason|rationale|purpose|explain|ipaliwanag)\b", clean):
            return False
        return bool(re.search(
            r"\b(?:which|what|ano|anong|alin|aling)\b.{0,40}"
            r"\b(?:rule|directive|requirement|guideline)\b|"
            r"\b(?:rule|directive|requirement|guideline)\b.{0,30}"
            r"\b(?:apply|applies|applicable|covers?|relevant|defines?)\b",
            clean,
        ))
    ...
    def should_use_llm_generation(
        ...
        *,
        ...
        # A Rule-level semantic match is source selection, not a hardcoded
        # compliance verdict.  Binary natural-language propositions therefore
        # receive exactly one grounded synthesis call over the resolved source.
        if any(item.get("_misra_semantic_resolver") is True for item in candidates) and cls.yes_no_intent(question):
            return True
    ...
    def build_generation_contract(
        ...
        for item in results or []:
            ...
            if not identifier:
                continue
            if item.get("_misra_semantic_resolver") is True:
                statement = cls._source_rule_statement(str(item.get("text", "") or ""))
                row = f"- {kind} {identifier} | SOURCE-RESOLVED REQUIREMENT: {statement}"
                rows.append(row)
                continue
        ...
        instructions = [
            ...
            *extra,
            ...
            "10. A SOURCE-RESOLVED REQUIREMENT establishes the relevant Rule/Directive but does not pre-decide the user's proposition. For a Yes/No question, derive Yes/No only from the exact requirement and the relation stated in the current question; if that relation is not determined by the source, answer Needs more context.",
            "11. Never answer 'Information not found' when a SOURCE-RESOLVED REQUIREMENT is present in the accepted context. The source has already been found; answer from it or say Needs more context for the requested relation.",
        ]
        return "\n".join(instructions)
    ...
    def should_isolate_generation_history(
        ...
        *,
        ...
        # semantic cue or explicit MISRA standards scope means the turn can be
        # answered from its own wording plus retrieved company evidence, without
        # borrowing previous code.
        return bool(cls.semantic_cues(raw) or ("misra" in raw.casefold()))
    ...
    def deterministic_requirement_lookup(cls, question: str, results: Sequence[Mapping]) -> str:
        ...
        if not cls.natural_requirement_lookup_intent(question) and not (
            semantic_resolved and cls.semantic_rule_lookup_intent(question)
        ):
            return ""
        candidates = [
            item for item in (results or [])
        ...
        for item in candidates:
            if (
                item.get("_misra_rule_body_rescue") is True
                and item.get("_misra_semantic_resolver") is not True
                and float(item.get("_misra_cue_coverage", 0.0) or 0.0) < 0.72
            ):
                continue
            metadata = item.get("metadata", {}) or {}
    ...
    def deterministic_scope_guard_answer(
        ...
        if any(
            ...
            # Corpus-semantic Rule selection identifies the authoritative Rule,
            # but this scope guard is not the proposition-relation verifier.
            # Return the same string contract as every other deterministic
            # finalizer so AnswerService never receives a truthy tuple here.
            return ""
    ...
    def answer_focus(cls, question: str = "") -> str:
        ...
        if cls.natural_requirement_lookup_intent(question) or cls.semantic_rule_lookup_intent(question):
            return (
