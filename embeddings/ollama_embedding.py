...
class OllamaEmbeddingModel:
    ...
    def get_query_embedding_batch(self, texts: Iterable[str]):
        """Embed multiple retrieval queries in one Ollama batch request.

        v6.5.0 uses this for original + MultiQuery semantic Rule resolution so
        recall expansion does not require one embedding HTTP round-trip per
        rewrite.  The same Qwen3 retrieval instruction is applied to every row.
        """
        return self._embed([self.QUERY_INSTRUCTION + str(text or "") for text in texts])
