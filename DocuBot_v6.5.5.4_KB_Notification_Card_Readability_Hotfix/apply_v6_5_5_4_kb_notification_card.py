...
VERSION = "v6.5.5.4"
SIMULATE = os.getenv("DOCUBOT_V6554_SIMULATE", "").strip() == "1"
FORCE_FAIL = os.getenv("DOCUBOT_V6554_FORCE_VALIDATION_FAIL", "").strip() == "1"
RESULT_REL = "logs/v6_5_5_4_notification_card"
MARKER_REL = "KB_NOTIFICATION_CARD_HOTFIX_APPLIED_v6.5.5.4.txt"
OLD_MANIFEST_REL = "FINAL_COMPONENT_MANIFEST_v6.5.5.3.json"
NEW_MANIFEST_REL = "FINAL_COMPONENT_MANIFEST_v6.5.5.4.json"
...
def verify_baseline(root: Path):
    ...
    for rel, wanted in (BASELINE.get("required_existing_sha256") or {}).items():
        p = root / rel
        if not p.is_file():
            problems.append(f"missing v6.5.5.3 baseline file: {rel}")
        else:
    ...
    for rel in BASELINE.get("required_absent") or []:
        if (root / rel).exists():
            problems.append(f"expected absent on clean v6.5.5.3 baseline: {rel}")
    return not problems, problems
...
def fail_restore(root, history, state, message, code):
    print("[FAIL]", message)
    print("[ROLLBACK] Restoring exact v6.5.5.3 files...")
...
def main():
    ...
    print("=" * 104)
    print("DocuBot v6.5.5.4 - KB Notification Card Readability Hotfix")
    print("=" * 104)
    print("[INFO] Scope: replace the clipped native success toast with a DocuBot-owned fixed notification card.")
    print("[INFO] v6.5.5.3 chat-disable behavior and v6.5.4+ incremental KB lifecycle are preserved.")
    ...
    if not ok:
        print("[FAIL] Project is not the exact supported v6.5.5.3 baseline. No files changed.")
        for x in problems:
            print(" -", x)
        return 3
    print(f"[PASS] Exact v6.5.5.3 baseline verified ({len(BASELINE['required_existing_sha256'])} covered files).")
    ...
    history = root / "evidence" / "release_history" / f"pre_v6.5.5.4_{stamp}"
    ...
    try:
        ...
        marker.write_text(
            "\n".join([
                ...
                "Base: exact verified v6.5.5.3",
                "Success notification card full text visibility: YES",
                "Success notification card counts full visibility: YES",
                "Success notification card technical details exposed: NO",
    ...
    validation = run_cmd(root, [py, "-X", "utf8", "-u", "scripts/validate_v6_5_5_4_kb_notification_card.py"])
    print(validation.stdout or "", end="")
    if FORCE_FAIL:
        validation = subprocess.CompletedProcess(validation.args, 99, stdout=(validation.stdout or "") + "\nFORCED TEST FAILURE\n")
    if validation.returncode != 0:
        return fail_restore(root, history, state, "v6.5.5.4 notification card validation failed.", 7)
    ...
    summary = {
        ...
        "success_notification_width_px": 470,
        "success_notification_renderer": "docubot_fixed_notification_card",
        ...
        "simulation": SIMULATE,
        "next": "Restart DocuBot and visually verify one KB update: the fixed notification card must show the full title and all Added/Updated/Deleted counts.",
    }
    summary_path = outdir / f"v6.5.5.4_notification_card_{stamp}.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    result_zip = outdir / f"DocuBot_v6.5.5.4_KB_Notification_Card_Readability_Hotfix_Result_{stamp}.zip"
    evidence = [
        summary_path,
        root / MARKER_REL,
        root / NEW_MANIFEST_REL,
        root / "logs" / "v6_5_5_4_notification_card" / "validation_latest.json",
        root / "docs" / "KB_NOTIFICATION_CARD_SMOKE_v6.5.5.4.txt",
    ]
    ...
    print("[PASS] v6.5.5.4 installation completed.")
