...
# The production knowledge root is intentionally singular. Historical
# data/all_documents references are evidence-only and are not active runtime
# inputs. All ingestion/update/health workflows use technical_documents.
...
# Phase 6 certified and promoted this exact v4 store. Superseded vector stores, v3 Qdrant
...
try:
    EMBED_OLLAMA_CONNECT_TIMEOUT = max(1, min(30, int(os.getenv("DOCUBOT_EMBED_CONNECT_TIMEOUT", "5"))))
except (TypeError, ValueError):
    EMBED_OLLAMA_CONNECT_TIMEOUT = 5
...
# =====================================================
# MULTI-QUERY RETRIEVAL — ADAPTIVE PRODUCTION PROFILE
# =====================================================
# v6.4.95 enables controlled MultiQuery only after exact/structured/BM25 fast
# paths have had a chance to answer.  Two alternative search formulations plus
# the original query are fused with reciprocal-rank fusion, then the existing
# BGE reranker and 0.55 confidence gate remain the final precision guard.
# Set DOCUBOT_MULTI_QUERY=0 for a single-query A/B baseline without code edits.
MULTI_QUERY_RETRIEVAL_ENABLED = _env_bool("DOCUBOT_MULTI_QUERY", True)
# v6.4.97 uses Ollama JSON-schema structured output for retrieval rewrites.
# Disable only for controlled compatibility/A-B testing; the guarded legacy
# text parser remains as a safe fallback if structured generation is unavailable.
MULTI_QUERY_STRUCTURED_OUTPUT_ENABLED = _env_bool(
    "DOCUBOT_MULTI_QUERY_STRUCTURED_OUTPUT",
    True,
)
try:
    MULTI_QUERY_VARIANT_COUNT = max(1, min(3, int(os.getenv("DOCUBOT_MULTI_QUERY_VARIANTS", "2"))))
except (TypeError, ValueError):
    MULTI_QUERY_VARIANT_COUNT = 2
try:
    MULTI_QUERY_MAX_QUERY_CHARS = max(80, min(1000, int(os.getenv("DOCUBOT_MULTI_QUERY_MAX_CHARS", "400"))))
except (TypeError, ValueError):
    MULTI_QUERY_MAX_QUERY_CHARS = 400
try:
    MULTI_QUERY_RRF_K = max(10, min(200, int(os.getenv("DOCUBOT_MULTI_QUERY_RRF_K", "60"))))
except (TypeError, ValueError):
    MULTI_QUERY_RRF_K = 60

# =====================================================
# PROMPT STRATEGY — FEW-SHOT + SILENT GROUNDED REASONING
# =====================================================
# v6.4.96 keeps factual grounding in retrieved company knowledge while adding
# behavior-only few-shot examples and a silent evidence-check discipline.
#
# These toggles exist for controlled A/B evaluation without source edits:
#   DOCUBOT_FEW_SHOT_PROMPT=0
#   DOCUBOT_REASONING_GUIDANCE=0
#
# "Reasoning guidance" never asks the model to reveal chain-of-thought. It only
# requires an internal evidence/polarity/scope check before returning the final
# grounded answer.
FEW_SHOT_PROMPTING_ENABLED = _env_bool("DOCUBOT_FEW_SHOT_PROMPT", True)
INTERNAL_REASONING_GUIDANCE_ENABLED = _env_bool(
    "DOCUBOT_REASONING_GUIDANCE",
    True,
)
PROMPT_FEW_SHOT_PROFILE = "behavior-only-v1"
