...
def _kb_update_success_message(update_result):
    build_action = str((update_result or {}).get("build_action") or "")
    if build_action == "transactional_incremental":
        details = (update_result or {}).get("build_details") or {}
        return (
...
if flash_message:
    # Toast is deliberately overlay-only so a completed update does not push
    # the welcome/composer region away from its normal position.
